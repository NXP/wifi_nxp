 ...
 */
#if defined(SDK_OS_FREE_RTOS)
#define NAL_PKT_2_BUF(p)    (p)
#define NAL_BUF_NEXT(p)     ((void *)(((struct pbuf *)(p))->next))
#define NAL_BUF_PAYLOAD(p)  ((void *)(((struct pbuf *)(p))->payload))
#define NAL_BUF_LEN(p)      (((struct pbuf *)(p))->len)
#define NAL_BUF_TOT_LEN(p)  (((struct pbuf *)(p))->tot_len)
#define NAL_PKT_HEAD_ADDR(p) NAL_BUF_PAYLOAD(p)
#define NAL_PKT_SET_IFACE(p, if_idx)
 ...
 */
static inline void *net_stack_buffer_alloc_rx(int offset, int len)
{
    return (void *)pbuf_alloc((pbuf_layer)offset, (uint16_t)len, PBUF_POOL);
}
 ...
 * \return zero ptr on failure
 */
static inline void *net_stack_buffer_clone_tx_frag(void *pkt, void *p, int offset)
{
    struct pbuf *tmp;

    tmp = pbuf_clone((pbuf_layer)offset, PBUF_RAM, (struct pbuf *)p);
    if (tmp == NULL)
    {
        tmp = pbuf_clone((pbuf_layer)offset, PBUF_POOL, (struct pbuf *)p);
    }
    return (void *)tmp;
}
 ...
 */
static inline int net_stack_buffer_push(void *p, int len)
{
    return pbuf_header((struct pbuf *)p, len);
}
 ...
 *
 */
static inline void net_stack_buffer_set_iface(void *p, int interface)
{
    return;
}
#elif __ZEPHYR__
...
{
#if defined(SDK_OS_FREE_RTOS)
    pbuf_realloc((struct pbuf *)p, len);
#elif __ZEPHYR__
