...
#endif
#ifdef CONFIG_ZEPHYR
extern struct k_timer wake_timer;
#else
...
void wlan_config_host_sleep(bool is_manual, t_u8 is_periodic)
{
    ...
    else
    {
        ...
        wlan_host_sleep_state = HOST_SLEEP_DISABLE;
#ifdef CONFIG_ZEPHYR
        if(k_timer_remaining_ticks(&wake_timer))
        {
...
int wlan_add_network(struct wlan_network *network)
{
    ...
    {
        (void)PRINTF("Not support 5G, please set 2G channel\n\r");
        return -WM_E_INVAL;
    }
    if ((network->acs_band == 1) && ISSUPP_NO5G(mlan_adap->fw_cap_ext))
    {
        (void)PRINTF("Not support 5G, please not set acs_band 1\n\r");
        return -WM_E_INVAL;
    }
