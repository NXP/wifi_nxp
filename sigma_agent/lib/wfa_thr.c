 ...
 */
static tgWMM_t wmm_thr[WFA_THREADS_NUM];
static char recvBuf[MAX_RCV_BUF_LEN + 1];
...
int tgSockfds[WFA_MAX_TRAFFIC_STREAMS] = {-1, -1, -1};
...
extern void tmout_stop_send(union sigval sv);
...
void tmout_stop_send(union sigval sv)
{
    struct timeval af;
    int i = 0;
...
void wfaSentStatsResp(int sock, BYTE *buf)
{
    ...
    for (i = 0; i < WFA_MAX_TRAFFIC_STREAMS; i++)
    {
        if ((allStreams->id != 0) && (allStreams->profile.direction == DIRECT_SEND) &&
            (allStreams->state == WFA_STREAM_ACTIVE))
        {
...
#if 1
    PRINTF("%u %u %llu %llu\r\n", first->cmdru.stats.txFrames, first->cmdru.stats.rxFrames,
           first->cmdru.stats.txPayloadBytes, first->cmdru.stats.rxPayloadBytes);
#endif

    wfaEncodeTLV(WFA_TRAFFIC_AGENT_SEND_RESP_TLV, total * sizeof(dutCmdResponse_t), (BYTE *)first, (BYTE *)buff);
...
{
    int myId             = idx++; //((tgThrData_t *)thr_param)->tid;
    ...
    memset(&my_wmm->sev, 0, sizeof(struct sigevent));
    memset(&my_wmm->trigger, 0, sizeof(struct itimerspec));
     ...
     *
     */
    my_wmm->sev.sigev_notify          = SIGEV_THREAD;
    my_wmm->sev.sigev_notify_function = &tmout_stop_send;
     ...
     * this timer.
     */
    timer_create(CLOCK_REALTIME, &my_wmm->sev, &my_wmm->timerid);
    ...
    while (1)
    {
        ...
        switch (myProfile->direction)
        {
            case DIRECT_SEND:
                ...
                {
                    my_wmm->trigger.it_value.tv_sec = myProfile->duration;
                    ;
                    /* Arm the timer. No flags are set and no old_value will be retrieved.
                     */
                    timer_settime(my_wmm->timerid, 0, &my_wmm->trigger, NULL);

                    DPRINT_INFO(WFA_OUT, "wfa_wmm_thread SEND set stop alarm for %d sec", myProfile->duration);
                }
                ...
                if (myProfile->profile == PROF_IPTV || myProfile->profile == PROF_FILE_TX ||
                    myProfile->profile == PROF_MCAST)
                {
                    int iOptVal, iOptLen;
                ...
                else if (myProfile->profile == PROF_TRANSC || myProfile->profile == PROF_START_SYNC ||
                         myProfile->profile == PROF_CALI_RTD)
                {
#if 0
                ...
                wFCNTL(mySock, F_SETFL, ioflags | O_NONBLOCK);
                    ...
                    while (gtgTransac != 0)
                    {
                        ...
                        if (gtgTransac != 0 /* && nbytes <= 0 */)
                        {
                            ...
                            if (wfaSendShortFile(mySock, myStreamId, trafficBuf, 0, respBuf, &respLen) == DONE)
                            {
                                if (wfaCtrlSend(gxcSockfd, respBuf, respLen) != respLen)
                                {
                                    DPRINT_INFO(
                                        WFA_OUT,
                                        "wfa_wmm_thread SEND,PROF_TRANSC::wfaCtrlSend Error for wfaSendShortFile\n");
                                }
                                sendFailCount++;
                                i--;
                                wUSLEEP(1000);
                            ...
                            {   /* Do not print any msg it will slow down process on snd/rcv  */
                                // setsockopt(mySock, SOL_SOCKET, SO_RCVTIMEO, (char *)&tmout, (socklen_t)
                                // sizeof(tmout)); printf("PROF_TRANSC::time out event, wfaRecvFile failed,resend a new
                                // packet ...\n");
                        ...
                        } while ((i <= myProfile->rate + myProfile->rate / 3) && (myProfile->rate != 0) &&
                                 (gtgTransac != 0));
                    ...
                    DPRINT_INFO(WFA_OUT,
                                "wfa_wmm_thread SEND::Sending stats back, sendCount=%d rcvCount=%d sleepTotal in "
                                "mil-sec=%d sendFailCount=%d frmRate=%d do count=%d\n",
            ...
            case DIRECT_RECV:
                ...
                else if (myProfile->profile == PROF_IPTV || myProfile->profile == PROF_FILE_TX ||
                         myProfile->profile == PROF_MCAST)
                {
                    int iOptVal, iOptLen;
                    struct timeval tmout;
                    ...
                    for (;;)
                    {
                        ...
                        if (myProfile->profile == PROF_IPTV)
                        {
                            ...
                            wGETTIMEOFDAY(&currTimeVal, NULL);
                    ...
                    if (myProfile->profile == PROF_IPTV)
                    {
                        ...
                        if (e2eoutp != NULL)
                        {
                            ...
                            for (j = 0; j < le2eCnt; j++)
                            {
                                ...
                                fprintf(e2eoutp, "%i:%i:%i:%i:%i\n", ep->seqnum, ep->lsec, ep->lusec, ep->rsec,
                                        ep->rusec);
                            }
                            fclose(e2eoutp);
                        }
