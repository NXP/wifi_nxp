...
int btSockfd = -1;
int adj_latency;
int vhtFlag = WFA_OFF; /* non VHT mode for traffic */
...
extern int wfaTrafficRecvNew(int, char *, struct sockaddr *, unsigned char *);
...
static int tableDscpToTos[16][2] = {{0, 0},    {8, 32},   {10, 40},  {14, 56},  {18, 72},  {22, 88},
                                    {24, 96},  {28, 112}, {30, 120}, {34, 136}, {36, 144}, {38, 152},
                                    {40, 160}, {46, 184}, {48, 192}, {56, 224}};
...
int getTrafficClass(unsigned char tos)
{
    //    int tostmp = (int)tos;
...
/** 6282 Box Optimization**/
int getACPrio(int trafficClass)
{
    switch (trafficClass)
    {
        case TG_WMM_AC_BK:
...
int isActiveHiPrioRxStreamPresent(int trafficClass)
{
    int i;
    tgStream_t *myStream = gStreams;
    ...
    for (i = 0; i < WFA_MAX_TRAFFIC_STREAMS; i++)
    {
        if ((myStream->profile.direction == DIRECT_RECV) && (myStream->state == WFA_STREAM_ACTIVE) &&
            (getACPrio(myStream->profile.trafficClass) >= getACPrio(trafficClass)))
        {
            /*	  printf("\n*************************\nmyStream->profile.direction %d myStream->state %d
             * myStream->profile.trafficClass %d trafficClass %d i %d \n **************************\n",
             * myStream->profile.direction, myStream->state, myStream->profile.trafficClass , trafficClass , i );*/
            return 1;
        }
...
int wfaTGSendPing(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    interval = 1000 / staPing->frameRate;
...
#if 1
    DPRINT_INFO(WFA_OUT,
                "profile %i direction %i dest ip %s dport %i source %s sport %i rate %i duration %i size %i class %i "
                "delay %i\n",
                myStream->profile.profile, myStream->profile.direction, myStream->profile.dipaddr,
                myStream->profile.dport, myStream->profile.sipaddr, myStream->profile.sport, myStream->profile.rate,
                myStream->profile.duration, myStream->profile.pksize, myStream->profile.trafficClass,
...
int wfaTGRecvStop(int len, BYTE *parms, int *respLen, BYTE *respBuf)
{
    ...
    DPRINT_INFO(WFA_OUT, "entering tgRecvStop with length %d", len);
...
#if 1
        DPRINT_INFO(WFA_OUT, "stream Id %u rx %u total %u\n", streamid, myStream->stats.rxFrames,
                    myStream->stats.rxPayloadBytes);
#endif
    ...
    for (i = 0; i < numStreams; i++)
    {
        ...
        switch (theProfile->profile)
        {
            ...
            case PROF_UAPSD:
            {
                ...
                wfaEncodeTLV(WFA_TRAFFIC_AGENT_SEND_RESP_TLV, sizeof(dutCmdResponse_t), (BYTE *)&staSendResp,
                             (BYTE *)respBuf);
                *respLen = WFA_TLV_HDR_LEN + sizeof(dutCmdResponse_t);
            }
            break;
...
int wfaSendLongFile(int mySockfd, int streamid, BYTE *aRespBuf, int *aRespLen)
{
    ...
    DPRINT_INFO(WFA_OUT, "Entering sendLongFile %i", streamid);
    ...
    else /* invalid parameters */
    {
        ...
        wfaEncodeTLV(WFA_TRAFFIC_AGENT_SEND_RESP_TLV, 4, (BYTE *)&sendResp, (BYTE *)aRespBuf);
...
#if 1
    DPRINT_INFO(WFA_OUT, "stream Id %u tx %u total %u\n", myStream->id, myStream->stats.txFrames,
                myStream->stats.txPayloadBytes);
#endif

    wfaEncodeTLV(WFA_TRAFFIC_AGENT_SEND_RESP_TLV, sizeof(dutCmdResponse_t), (BYTE *)&sendResp, (BYTE *)aRespBuf);
...
int wfaSendBitrateData(int mySockfd, int streamId, BYTE *pRespBuf, int *pRespLen)
{
    ...
    int counter = 0, i, iSleep = 0;                                     /*  frame data sending count */
    ...
    {
        DPRINT_INFO(WFA_OUT,
                    "wfaSendBitrateData pass-in parameter err mySockfd=%i streamId=%i pRespBuf=0x%x pRespLen=0x%x\n",
    ...
    {
        DPRINT_INFO(WFA_OUT,
                    "wfaSendBitrateData Warning, must set total streams rate<=10000 and payload<=1000; stream bitrate "
                    "may not meet\n");
    }
    ...
    while (runLoop)
    {
        ...
        for (i = 0; i <= (theProf->rate); i++)
        {
            ...
            else
            {
                ...
                DPRINT_INFO(WFA_OUT,
                            "wfaSendBitrateData wfaTrafficSendTo call ERR counter=%i i=%i; send busy, sleep %i MilSec "
                            "then send\n",
    ...
    *pRespLen = WFA_TLV_HDR_LEN + sizeof(dutCmdResponse_t);
    ...
    DPRINT_INFO(WFA_OUT,
                "*** wfg_tg.cpp wfaSendBitrateData Count=%i txFrames=%i totalByteSent=%i sleepTotal=%llu milSec "
                "extraTimeSpendOnSending=%llu nOverTimeCount=%d nOverSend=%i rate=%d nDuration=%d iSleep=%d ***\n",
                counter, (myStream->stats.txFrames), (unsigned int)(myStream->stats.txPayloadBytes), sleepTotal,
                extraTimeSpendOnSending, nOverTimeCount, nOverSend, theProf->rate, nDuration, iSleep);
    wfaSleepMilsec(1000);
    return ret;
