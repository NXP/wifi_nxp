 ...
 *  @return		N/A
 */
static void wlan_enable_aes_key(pmlan_private pmpriv)
{
 ...
 */
static mlan_status wlan_get_info_signal(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_RSSI_INFO, HostCmd_ACT_GEN_GET, 0, (t_void *)pioctl_req, MNULL);
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_get_info_stats(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    pmlan_private pmpriv = MNULL;
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_GET_LOG, HostCmd_ACT_GEN_GET, 0, (t_void *)pioctl_req, MNULL);
 ...
 */
static mlan_status wlan_get_info_bss_info(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    {
        tbl_idx = wlan_find_ssid_in_list(pmpriv, &pbss_desc->ssid, pbss_desc->mac_address, pmpriv->bss_mode);
        if (tbl_idx >= 0)
    ...
    if (pmpriv->assoc_rsp_buf)
        info->param.bss_info.assoc_id = (t_u16)((IEEEtypes_AssocRsp_t *)pmpriv->assoc_rsp_buf)->a_id;
    else
 ...
 */
static mlan_status wlan_get_info_ioctl(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
...
static mlan_status wlan_radio_ioctl_band_cfg(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_SET)
    {
        ...
        if (wlan_set_regiontable(pmpriv, (t_u8)pmadapter->region_code, global_band | adhoc_band))
        {
        ...
        pmadapter->chan_bandwidth                  = (t_u8)radio_cfg->param.band_cfg.sec_chan_offset;
        /*
        ...
        if (!adhoc_channel)
        {
            if (!wlan_find_cfp_by_band_and_channel(pmadapter, pmadapter->adhoc_start_band, pmpriv->adhoc_channel))
            {
                /* Pass back the default channel */
                radio_cfg->param.band_cfg.adhoc_channel = DEFAULT_AD_HOC_CHANNEL;
                if ((pmadapter->adhoc_start_band & BAND_A) || (pmadapter->adhoc_start_band & BAND_AN))
                {
                    radio_cfg->param.band_cfg.adhoc_channel = DEFAULT_AD_HOC_CHANNEL_A;
                }
            }
        }
        else
        { /* Return error if adhoc_band and adhoc_channel
            ...
            if (!wlan_find_cfp_by_band_and_channel(pmadapter, pmadapter->adhoc_start_band, (t_u16)adhoc_channel))
            {
                pioctl_req->status_code = MLAN_ERROR_INVALID_PARAMETER;
                LEAVE();
                return MLAN_STATUS_FAILURE;
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_bss_ioctl_mac_address(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
 ...
 */
static mlan_status wlan_bss_ioctl_set_multicast_list(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_bss *bss     = MNULL;
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    ...
    pioctl_req->data_read_written = sizeof(mlan_multicast_list) + MLAN_SUB_COMMAND_SIZE;
    ...
    else
    {
        ...
        else
        {
            ...
            if (bss->param.multicast_list.num_multicast_addr)
            {
                ...
                {
                    /* Send request to firmware */
                    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_MAC_MULTICAST_ADR, HostCmd_ACT_GEN_SET, 0,
                                           (t_void *)pioctl_req, &bss->param.multicast_list);
                    if (ret == MLAN_STATUS_SUCCESS)
                        ret = MLAN_STATUS_PENDING;
                }
                else
                {
                    /* Send request to firmware */
                    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_MAC_MULTICAST_ADR, HostCmd_ACT_GEN_SET, 0, MNULL,
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_bss_ioctl_get_channel_list(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_bss *bss     = MNULL;
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    ...
    if ((wlan_11d_is_enabled(pmpriv) && pmpriv->media_connected == MTRUE) &&
        ((pmpriv->bss_mode == MLAN_BSS_MODE_INFRA) ||
         (pmpriv->bss_mode == MLAN_BSS_MODE_IBSS && pmpriv->adhoc_state != ADHOC_STARTED)))
    {
        t_u8 chan_no;
        t_u8 band;
        ...
        for (i = 0; (bss->param.chanlist.num_of_chan < MLAN_MAX_CHANNEL_NUM) && (i < parsed_region_chan->no_of_chan);
             i++)
        {
            chan_no = parsed_region_chan->chan_pwr[i].chan;
            band    = parsed_region_chan->chan_pwr[i].band;
            PRINTM(MINFO, "band=%d, chan_no=%d\n", band, chan_no);
            bss->param.chanlist.cf[bss->param.chanlist.num_of_chan].channel = (t_u32)chan_no;
    ...
    {
        for (j = 0; (bss->param.chanlist.num_of_chan < MLAN_MAX_CHANNEL_NUM) && (j < MAX_REGION_CHANNEL_NUM); j++)
        {
            cfp = pmadapter->region_channel[j].pcfp;
            for (i = 0; (bss->param.chanlist.num_of_chan < MLAN_MAX_CHANNEL_NUM) &&
                        pmadapter->region_channel[j].valid && cfp && (i < pmadapter->region_channel[j].num_cfp);
                 i++)
            {
                bss->param.chanlist.cf[bss->param.chanlist.num_of_chan].channel = (t_u32)cfp->channel;
                bss->param.chanlist.cf[bss->param.chanlist.num_of_chan].freq    = (t_u32)cfp->freq;
                bss->param.chanlist.num_of_chan++;
                cfp++;
            }
        }
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_bss_ioctl_channel(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv   = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_bss *bss       = MNULL;
    mlan_status ret        = MLAN_STATUS_SUCCESS;
    ...
    if (pioctl_req->action == MLAN_ACT_GET)
    {
        cfp = wlan_find_cfp_by_band_and_channel(pmadapter, pmpriv->curr_bss_params.band,
        ...
        pioctl_req->data_read_written = sizeof(chan_freq) + MLAN_SUB_COMMAND_SIZE;
        LEAVE();
        return ret;
    }
    ...
    if (bss->param.bss_chan.channel)
    {
        if (bss->param.bss_chan.channel <= MAX_CHANNEL_BAND_B)
            cfp = wlan_find_cfp_by_band_and_channel(pmadapter, BAND_B, (t_u16)bss->param.bss_chan.channel);
        if (!cfp)
        {
            cfp = wlan_find_cfp_by_band_and_channel(pmadapter, BAND_A, (t_u16)bss->param.bss_chan.channel);
            if (cfp)
            {
    ...
    else
    {
        if (bss->param.bss_chan.freq <= MAX_FREQUENCY_BAND_B)
            cfp = wlan_find_cfp_by_band_and_freq(pmadapter, BAND_B, bss->param.bss_chan.freq);
        if (!cfp)
        {
            cfp = wlan_find_cfp_by_band_and_freq(pmadapter, BAND_A, bss->param.bss_chan.freq);
            if (cfp)
            {
                if (pmadapter->adhoc_11n_enabled)
                    pmadapter->adhoc_start_band = BAND_A | BAND_AN;
 ...
 *  @return		MLAN_STATUS_SUCCESS/MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_bss_ioctl_mode(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_bss *bss     = MNULL;
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    ...
    if ((pmpriv->bss_mode == bss->param.bss_mode) || (bss->param.bss_mode == MLAN_BSS_MODE_AUTO))
    {
    ...
    if ((pmpriv->bss_mode != MLAN_BSS_MODE_AUTO))
    {
        ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_SET_BSS_MODE, HostCmd_ACT_GEN_SET, 0, (t_void *)pioctl_req, MNULL);
        if (ret == MLAN_STATUS_SUCCESS)
            ret = MLAN_STATUS_PENDING;
    }
exit:
...
static mlan_status wlan_bss_ioctl_start(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pmpriv->bss_mode == MLAN_BSS_MODE_INFRA)
    {
        ...
        {   /* i >= 0 */
            ...
            PRINTM(MERROR, "SSID not found in scan list: ssid=%s, %02x:%02x:%02x:%02x:%02x:%02x, idx=%d\n",
                   ...
                   bss->param.ssid_bssid.bssid[5], (int)bss->param.ssid_bssid.idx);
            pioctl_req->status_code = MLAN_ERROR_INVALID_PARAMETER;
            ret                     = MLAN_STATUS_FAILURE;
            goto start_ssid_done;
#endif /* CONFIG_MLAN_WMSDK */
    ...
    else
    {
        ...
        if (bss->param.ssid_bssid.ssid.ssid_len &&
            (!wlan_ssid_cmp(pmadapter, &pmpriv->curr_bss_params.bss_descriptor.ssid, &bss->param.ssid_bssid.ssid)))
        {
            ret = MLAN_STATUS_SUCCESS;
            goto start_ssid_done;
        }
        ...
        if (!bss->param.ssid_bssid.idx || bss->param.ssid_bssid.idx > pmadapter->num_in_scan_table)
        {
            ...
            {
                i = wlan_find_bssid_in_list(pmpriv, (t_u8 *)&bss->param.ssid_bssid.bssid, MLAN_BSS_MODE_IBSS);
            }
        }
        else
        {
        ...
        if (i >= 0)
        {
            ...
            ret = wlan_adhoc_join(pmpriv, pioctl_req, &pmadapter->pscan_table[i]);
            if (ret)
                goto start_ssid_done;
        ...
        { /* i >= 0 */
            ...
            ret = wlan_adhoc_start(pmpriv, pioctl_req, &bss->param.ssid_bssid.ssid);
            if (ret)
                goto start_ssid_done;
            if (pmpriv->adhoc_aes_enabled)
                wlan_enable_aes_key(pmpriv);
 ...
 *  @return		MLAN_STATUS_SUCCESS/MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_bss_ioctl_stop(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    pmlan_private pmpriv = pmadapter->priv[pioctl_req->bss_index];
 ...
 *  @return		MLAN_STATUS_SUCCESS/MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_bss_ioctl_ibss_channel(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_bss *bss     = MNULL;
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_RF_CHANNEL, cmd_action, 0, (t_void *)pioctl_req,
 ...
 *  @return		MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_bss_ioctl_beacon_interval(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_bss *bss     = MNULL;
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    ...
    if (pioctl_req->action == MLAN_ACT_GET)
    {
        ...
        if (pmpriv->media_connected == MTRUE)
            bss->param.bcn_interval = pmpriv->curr_bss_params.bss_descriptor.beacon_period;
    }
    else
 ...
 *  @return		MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_bss_ioctl_atim_window(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_bss *bss     = MNULL;
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    ...
    if (pioctl_req->action == MLAN_ACT_GET)
    {
        ...
        if (pmpriv->media_connected == MTRUE)
            bss->param.atim_window = pmpriv->curr_bss_params.bss_descriptor.atim_window;
    }
    else
 ...
 *  @return                     MLAN_STATUS_SUCCESS -- success, otherwise fail
 */
static mlan_status wlan_query_passphrase(mlan_private *priv, pmlan_ioctl_req pioctl_req)
{
    ...
    {
        i = wlan_find_bssid_in_list(priv, (t_u8 *)&ssid_bssid->bssid, MLAN_BSS_MODE_AUTO);
        if (i >= 0)
        {
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(priv, HostCmd_CMD_SUPPLICANT_PMK, HostCmd_ACT_GEN_GET, 0, (t_void *)pioctl_req, &sec_pp);
 ...
 *  @return		MLAN_STATUS_SUCCESS/MLAN_STATUS_PENDING --success, otherwise fail
 */
mlan_status wlan_bss_ioctl_find_bss(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_status ret      = MLAN_STATUS_SUCCESS;
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_rate_ioctl_get_supported_rate(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    ...
    if (rate->param.rate_band_cfg.config_bands && rate->param.rate_band_cfg.bss_mode)
        wlan_get_active_data_rates(pmpriv, rate->param.rate_band_cfg.bss_mode, rate->param.rate_band_cfg.config_bands,
                                   rate->param.rates);
    else
        wlan_get_active_data_rates(
            pmpriv, pmpriv->bss_mode,
            (pmpriv->bss_mode == MLAN_BSS_MODE_INFRA) ? pmpriv->config_bands : pmadapter->adhoc_start_band,
            rate->param.rates);
    pioctl_req->data_read_written = MLAN_SUPPORTED_RATES + MLAN_SUB_COMMAND_SIZE;
    LEAVE();
    return ret;
}
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_rate_ioctl_get_data_rate(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    ...
    /* Send request to firmware */
    ret =
        wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_TX_RATE_QUERY, HostCmd_ACT_GEN_GET, 0, (t_void *)pioctl_req, MNULL);
    if (ret == MLAN_STATUS_SUCCESS)
        ret = MLAN_STATUS_PENDING;
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_pm_ioctl_ps_mode(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req, IN t_u16 ps_mode)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    t_u16 sub_cmd;
    ...
    {
        ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_PS_MODE_ENH, GET_PS, 0, (t_void *)pioctl_req, MNULL);
    }
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_pm_ioctl_inactivity_timeout(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv  = pmadapter->priv[pioctl_req->bss_index];
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_INACTIVITY_TIMEOUT_EXT, cmd_action, 0, (t_void *)pioctl_req,
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_set_auto_deep_sleep(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    pmlan_private pmpriv = (pmlan_private)pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_auto_ds auto_ds;
    t_u32 mode;

    ENTER();

    if (((mlan_ds_pm_cfg *)pioctl_req->pbuf)->param.auto_deep_sleep.auto_ds == DEEP_SLEEP_ON)
    {
    ...
    if (((mlan_ds_pm_cfg *)pioctl_req->pbuf)->param.auto_deep_sleep.idletime)
        auto_ds.idletime = ((mlan_ds_pm_cfg *)pioctl_req->pbuf)->param.auto_deep_sleep.idletime;
    else
        auto_ds.idletime = pmadapter->idle_time;
    /* note: the command could be queued and executed later if there is
       command in progress. */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_PS_MODE_ENH, (t_u16)mode, BITMAP_AUTO_DS, (t_void *)pioctl_req,
                           &auto_ds);
    if (ret)
    {
        LEAVE();
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_set_get_sleep_pd(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret        = MLAN_STATUS_SUCCESS;
    pmlan_private pmpriv   = pmadapter->priv[pioctl_req->bss_index];
    ...
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_SLEEP_PERIOD, cmd_action, 0, (t_void *)pioctl_req, &sleep_pd);
 ...
 *  @return		MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_set_get_ps_cfg(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_ds_pm_cfg *pm_cfg = MNULL;
    ...
    if (pioctl_req->action == MLAN_ACT_GET)
    {
        pm_cfg->param.ps_cfg.ps_null_interval       = (t_u32)pmadapter->null_pkt_interval;
        pm_cfg->param.ps_cfg.multiple_dtim_interval = (t_u32)pmadapter->multiple_dtim;
        pm_cfg->param.ps_cfg.listen_interval        = (t_u32)pmadapter->local_listen_interval;
        pm_cfg->param.ps_cfg.adhoc_awake_period     = (t_u32)pmadapter->adhoc_awake_period;
        pm_cfg->param.ps_cfg.bcn_miss_timeout       = (t_u32)pmadapter->bcn_miss_time_out;
    ...
    else
    {
        if (pm_cfg->param.ps_cfg.ps_null_interval)
            pmadapter->null_pkt_interval = (t_u16)pm_cfg->param.ps_cfg.ps_null_interval;
        else
            pm_cfg->param.ps_cfg.ps_null_interval = (t_u32)pmadapter->null_pkt_interval;
        if (pm_cfg->param.ps_cfg.multiple_dtim_interval)
            pmadapter->multiple_dtim = (t_u16)pm_cfg->param.ps_cfg.multiple_dtim_interval;
        else
            pm_cfg->param.ps_cfg.multiple_dtim_interval = (t_u32)pmadapter->multiple_dtim;
        if (((t_s32)pm_cfg->param.ps_cfg.listen_interval) == MRVDRV_LISTEN_INTERVAL_DISABLE)
            pmadapter->local_listen_interval = 0;
        else if (pm_cfg->param.ps_cfg.listen_interval)
            pmadapter->local_listen_interval = (t_u16)pm_cfg->param.ps_cfg.listen_interval;
        else
            pm_cfg->param.ps_cfg.listen_interval = (t_u32)pmadapter->local_listen_interval;
        if (pm_cfg->param.ps_cfg.adhoc_awake_period)
            pmadapter->adhoc_awake_period = (t_u16)pm_cfg->param.ps_cfg.adhoc_awake_period;
        else
            pm_cfg->param.ps_cfg.adhoc_awake_period = (t_u32)pmadapter->adhoc_awake_period;
        if (pm_cfg->param.ps_cfg.bcn_miss_timeout)
            pmadapter->bcn_miss_time_out = (t_u16)pm_cfg->param.ps_cfg.bcn_miss_timeout;
        else
            pm_cfg->param.ps_cfg.bcn_miss_timeout = (t_u32)pmadapter->bcn_miss_time_out;
 ...
 *  @return		        MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_set_get_sleep_params(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret        = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv   = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_pm_cfg *pm_cfg = MNULL;
    ...
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_SLEEP_PARAMS, cmd_action, 0, (t_void *)pioctl_req,
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_pm_ioctl(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status status = MLAN_STATUS_SUCCESS;
    ...
    switch (pm->sub_command)
    {
        case MLAN_OID_PM_CFG_IEEE_PS:
            switch (pioctl_req->action)
            {
                case MLAN_ACT_SET:
                    ...
                    status = wlan_pm_ioctl_ps_mode(pmadapter, pioctl_req, pmadapter->ps_mode);
                    break;
                case MLAN_ACT_GET:
                    status = wlan_pm_ioctl_ps_mode(pmadapter, pioctl_req, pmadapter->ps_mode);
                    break;
        ...
        case MLAN_OID_PM_CFG_DEEP_SLEEP:
            ...
            {
                case MLAN_ACT_SET:
                    if (pmadapter->is_deep_sleep && pm->param.auto_deep_sleep.auto_ds == DEEP_SLEEP_ON)
                    {
                        PRINTM(MMSG, "Station already in enhanced deep sleep mode\n");
                        status = MLAN_STATUS_FAILURE;
                        break;
                    }
                    else if (!pmadapter->is_deep_sleep && pm->param.auto_deep_sleep.auto_ds == DEEP_SLEEP_OFF)
                    {
                        PRINTM(MMSG, "Station already not in enhanced deep sleep mode\n");
                        status = MLAN_STATUS_FAILURE;
                        break;
                    }
 ...
 *  @return		MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_wmm_ioctl_enable(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
 ...
 *  @return             MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_wmm_ioctl_addts_req(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    pmlan_private pmpriv = pmadapter->priv[pioctl_req->bss_index];
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_WMM_ADDTS_REQ, 0, 0, (t_void *)pioctl_req, (t_void *)&cfg->param.addts);
 ...
 *  @return             MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_wmm_ioctl_delts_req(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    pmlan_private pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_wmm_cfg *cfg = MNULL;
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_WMM_DELTS_REQ, 0, 0, (t_void *)pioctl_req, (t_void *)&cfg->param.delts);
 ...
 *  @return             MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_wmm_ioctl_queue_config(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    pmlan_private pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_wmm_cfg *cfg = MNULL;
    ...
    /* Send request to firmware */
    ret =
        wlan_prepare_cmd(pmpriv, HostCmd_CMD_WMM_QUEUE_CONFIG, 0, 0, (t_void *)pioctl_req, (t_void *)&cfg->param.q_cfg);
 ...
 *  @return             MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_wmm_ioctl_queue_stats(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    pmlan_private pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_wmm_cfg *cfg = MNULL;
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_WMM_QUEUE_STATS, 0, 0, (t_void *)pioctl_req,
 ...
 *  @return             MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_wmm_ioctl_queue_status(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret                    = MLAN_STATUS_SUCCESS;
    pmlan_private pmpriv               = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_wmm_cfg *cfg               = MNULL;
 ...
 *  @return             MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_wmm_ioctl_ts_status(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    pmlan_private pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_wmm_cfg *cfg = MNULL;
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_WMM_TS_STATUS, 0, 0, (t_void *)pioctl_req,
 ...
 *  @return             MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_set_wapi_ie(mlan_private *priv, t_u8 *ie_data_ptr, t_u16 ie_len)
{
    ENTER();
    if (ie_len)
    {
 ...
 *  @return		MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_sec_ioctl_wapi_enable(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_sec_ioctl_set_wapi_key(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_sec_cfg *sec = MNULL;
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_sec_ioctl_port_ctrl_enable(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_ds_sec_cfg *sec = MNULL;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
 ...
 *  @return		MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_sec_ioctl_auth_mode(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_sec_cfg *sec = MNULL;
 ...
 *  @return		MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_sec_ioctl_encrypt_mode(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_sec_cfg *sec = MNULL;
 ...
 *  @return		MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_sec_ioctl_wpa_enable(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_sec_cfg *sec = MNULL;
...
#endif
#else
        ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_SET_WEP, HostCmd_ACT_ADD, 0, (t_void *)pioctl_req, MNULL);
    ...
    else
    {
        if (pwep_key->key_length != 0U)
        {
            ...
            ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_KEY_MATERIAL, HostCmd_ACT_GEN_SET, 0, MNULL,
                                   &sec->param.encrypt_key);
#else
            ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_KEY_MATERIAL, HostCmd_ACT_GEN_SET, 0, MNULL, MNULL);
#endif
#else
            ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_SET_WEP, HostCmd_ACT_ADD, 0, MNULL, MNULL);
#endif /* KEY_MATERIAL_WEP */
            if (ret != MLAN_STATUS_SUCCESS)
            {
                goto exit;
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_sec_ioctl_get_key(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_sec_cfg *sec = MNULL;
    ...
    if ((pmpriv->sec_info.wep_status == Wlan802_11WEPEnabled) ||
        (pmpriv->sec_info.wep_status == Wlan802_11WEPKeyAbsent) || pmpriv->sec_info.ewpa_enabled ||
        pmpriv->sec_info.wpa_enabled || pmpriv->sec_info.wpa2_enabled || pmpriv->adhoc_aes_enabled)
    {
    ...
    if (sec->param.encrypt_key.key_index == MLAN_KEY_INDEX_DEFAULT)
    {
            ...
            (pmpriv->sec_info.wep_status == Wlan802_11WEPEnabled))
        {
            ...
            sec->param.encrypt_key.key_len = MIN(MLAN_MAX_KEY_LENGTH, pmpriv->wep_key[index].key_length);
                 ...
                 (pmpriv->sec_info.wapi_enabled) || (pmpriv->adhoc_aes_enabled))
        {
            /* Return WPA enabled */
            sec->param.encrypt_key.key_disable = MFALSE;
            (void)__memcpy(pmadapter, sec->param.encrypt_key.key_material, pmpriv->aes_key.key_param_set.key,
 ...
 *  @return     MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_sec_ioctl_query_key(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_sec_cfg *sec = MNULL;
    ...
    if (pmpriv->port_open &&
        (pmpriv->sec_info.wpa_enabled || pmpriv->sec_info.wpa2_enabled || pmpriv->sec_info.wapi_enabled))
    {
        /* Send request to firmware */
        ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_KEY_MATERIAL, HostCmd_ACT_GEN_GET, KEY_INFO_ENABLED,
                               (t_void *)pioctl_req, &sec->param.encrypt_key);
 ...
 *  @return		MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_sec_ioctl_ewpa_enable(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_sec_cfg *sec = MNULL;
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_sec_ioctl_esupp_mode(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    t_u16 cmd_action     = 0;
    ...
    if (pioctl_req->action == MLAN_ACT_SET)
    {
        ...
        if (pmpriv->media_connected == MTRUE)
        {
            PRINTM(MERROR, "Cannot set esupplicant mode configuration while connected.\n");
            pioctl_req->status_code = MLAN_ERROR_IOCTL_INVALID;
            ret                     = MLAN_STATUS_FAILURE;
            goto exit;
        }
        if (!sec->param.esupp_mode.rsn_mode ||
            (sec->param.esupp_mode.rsn_mode & RSN_TYPE_VALID_BITS) != sec->param.esupp_mode.rsn_mode)
        {
        ...
        if (!sec->param.esupp_mode.act_paircipher ||
            (sec->param.esupp_mode.act_paircipher & EMBED_CIPHER_VALID_BITS) != sec->param.esupp_mode.act_paircipher)
        {
        ...
        if (!sec->param.esupp_mode.act_groupcipher ||
            (sec->param.esupp_mode.act_groupcipher & EMBED_CIPHER_VALID_BITS) != sec->param.esupp_mode.act_groupcipher)
        {
    ...
    if (sec)
    {
        ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_SUPPLICANT_PROFILE, cmd_action, 0, (t_void *)pioctl_req,
                               &sec->param.esupp_mode);
    }
    else
    {
        ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_SUPPLICANT_PROFILE, cmd_action, 0, (t_void *)pioctl_req, MNULL);
    }
    if (ret == MLAN_STATUS_SUCCESS)
        ret = MLAN_STATUS_PENDING;
 ...
 *  @return             MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static int wlan_set_gen_ie_helper(mlan_private *priv, t_u8 *ie_data_ptr, t_u16 ie_len)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_misc_ioctl_wws_cfg(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    pmlan_private pmpriv       = pmadapter->priv[pioctl_req->bss_index];
    mlan_status ret            = MLAN_STATUS_SUCCESS;
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_SNMP_MIB, cmd_action, WwsMode_i, (t_void *)pioctl_req, &enable);
 ...
 *  @return		MLAN_STATUS_SUCCESS/MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_11d_cfg_ioctl_enable(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret           = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv      = pmadapter->priv[pioctl_req->bss_index];
    ...
    if (pioctl_req->action == MLAN_ACT_SET)
    {
        if (pmpriv->media_connected == MTRUE)
        {
            PRINTM(MIOCTL, "11D setting cannot be changed while interface is active.\n");
            ret = MLAN_STATUS_FAILURE;
            goto done;
        }
        ...
        {
            ret = wlan_11d_enable(pmpriv, pioctl_req, (state_11d_t)pcfg_11d->param.enable_11d);
            if (ret == MLAN_STATUS_SUCCESS)
                ret = MLAN_STATUS_PENDING;
        }
        else
 ...
 *  @return		MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_11d_clr_chan_table(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    ...
    if (pioctl_req->action == MLAN_ACT_SET)
    {
        ...
        if (wlan_11d_clear_parsedtable(pmpriv) == MLAN_STATUS_SUCCESS)
            PRINTM(MINFO, "11D: cleared parsed_region_chan (now no_of_chan=%d)\n",
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_reg_mem_ioctl_reg_rw(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv     = pmadapter->priv[pioctl_req->bss_index];
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, cmd_no, cmd_action, 0, (t_void *)pioctl_req, (t_void *)&reg_mem->param.reg_rw);
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_reg_mem_ioctl_read_eeprom(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv     = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_reg_mem *reg_mem = MNULL;
    mlan_status ret          = MLAN_STATUS_SUCCESS;
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_EEPROM_ACCESS, cmd_action, 0, (t_void *)pioctl_req,
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_reg_mem_ioctl_mem_rw(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv     = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_reg_mem *reg_mem = MNULL;
    mlan_status ret          = MLAN_STATUS_SUCCESS;
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_MEM_ACCESS, cmd_action, 0, (t_void *)pioctl_req,
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_reg_mem_ioctl(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status status       = MLAN_STATUS_SUCCESS;
    mlan_ds_reg_mem *reg_mem = MNULL;
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_11h_channel_check_req(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    pmlan_private pmpriv = MNULL;
    ...
    if ((pmadapter->adhoc_start_band & BAND_A) || (pmadapter->adhoc_start_band & BAND_AN))
    {
            ...
            !wlan_11h_is_channel_under_nop(pmadapter, pmpriv->adhoc_channel))
        {
            ...
            ret = wlan_11h_issue_radar_detect(pmpriv, pioctl_req, pmpriv->adhoc_channel);
            if (ret == MLAN_STATUS_SUCCESS)
                ret = MLAN_STATUS_PENDING;
        }
    }
 ...
 *  @return		MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_11h_ioctl_local_power_constraint(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
 ...
 *  @return		MLAN_STATUS_SUCCESS/MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_11h_cfg_ioctl(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status status         = MLAN_STATUS_SUCCESS;
    mlan_ds_11h_cfg *ds_11hcfg = MNULL;
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_misc_ioctl_gen_ie(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret        = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv   = pmadapter->priv[pioctl_req->bss_index];
    ...
    switch (misc->param.gen_ie.type)
    {
        case MLAN_IE_TYPE_GEN_IE:
            ...
            {
                wlan_set_gen_ie_helper(pmpriv, misc->param.gen_ie.ie_data, (t_u16)misc->param.gen_ie.len);
            }
            break;
    ...
    pioctl_req->data_read_written = sizeof(mlan_ds_misc_gen_ie) + MLAN_SUB_COMMAND_SIZE;
    LEAVE();
    return ret;
}
#endif /* CONFIG_MLAN_WMSDK */
 ...
 *  @return		MLAN_STATUS_PENDING
 */
static mlan_status wlan_misc_ioctl_warm_reset(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    pmlan_private pmpriv = pmadapter->priv[pioctl_req->bss_index];
    t_s32 i              = 0;
    ...
    wlan_prepare_cmd(pmpriv, HostCmd_CMD_FUNC_SHUTDOWN, HostCmd_ACT_GEN_SET, 0, MNULL, MNULL);
 ...
 *  @return		MLAN_STATUS_SUCCESS --success, otherwise fail
 */
static mlan_status wlan_misc_ioctl_sdio_mpa_ctrl(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret                      = MLAN_STATUS_SUCCESS;
    pmlan_private pmpriv                 = pmadapter->priv[pioctl_req->bss_index];
    ...
    if (pioctl_req->action == MLAN_ACT_SET)
    {
        ...
        if (mpa_ctrl->tx_buf_size || mpa_ctrl->rx_buf_size)
        {
            ...
            if (wlan_alloc_sdio_mpa_buffers(pmadapter, pmadapter->mpa_tx.buf_size, pmadapter->mpa_rx.buf_size) !=
                MLAN_STATUS_SUCCESS)
            {
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_misc_ioctl_sysclock(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv   = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_misc_cfg *misc = MNULL;
    mlan_status ret        = MLAN_STATUS_SUCCESS;
    ...
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_ECL_SYSTEM_CLOCK_CONFIG, cmd_action, 0, (t_void *)pioctl_req,
 ...
 *  @return		MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_misc_ioctl_soft_reset(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    ...
    ENTER();
    /* Send command to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_SOFT_RESET, HostCmd_ACT_GEN_SET, 0, (t_void *)pioctl_req, MNULL);
 ...
 *  @return             MLAN_STATUS_PENDING -- success, otherwise fail
 */
mlan_status wlan_misc_ioctl_mac_control(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv   = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_misc_cfg *misc = (mlan_ds_misc_cfg *)pioctl_req->pbuf;
    mlan_status ret        = MLAN_STATUS_SUCCESS;
    ...
    else
    {
        ...
        /* Send command to firmware */
        ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_MAC_CONTROL, cmd_action, 0, (t_void *)pioctl_req,
                               &misc->param.mac_ctrl);
 ...
 *  @return             MLAN_STATUS_PENDING -- success, otherwise fail
 */
mlan_status wlan_misc_ioctl_thermal(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv   = pmadapter->priv[pioctl_req->bss_index];
    mlan_status ret        = MLAN_STATUS_SUCCESS;
    mlan_ds_misc_cfg *misc = MNULL;
    ...
    /* Send command to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_SNMP_MIB, cmd_action, Thermal_i, (t_void *)pioctl_req, MNULL);
 ...
 *  @return             MLAN_STATUS_PENDING --success, otherwise fail
 */
mlan_status wlan_ipaddr_arp_filter(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req, IN t_u32 ipv4_addr)
{
    mlan_status ret     = MLAN_STATUS_SUCCESS;
    pmlan_callbacks pcb = &pmadapter->callbacks;
    t_u8 *buf;
    ...
    pcb->moal_malloc(pmadapter->pmoal_handle, MRVDRV_SIZE_OF_CMD_BUFFER, MLAN_MEM_DEF, &buf);
    if (!buf)
    {
        ret = MLAN_STATUS_FAILURE;
        goto done;
 ...
 *  @return             MLAN_STATUS_PENDING --success, otherwise fail
 */
mlan_status wlan_ipaddr_auto_arp_resp(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req, IN t_u32 ipv4_addr)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_status ret      = MLAN_STATUS_SUCCESS;
    ...
    t_u32 buf_len = 0;
    t_u8 *buf, *filter;
    t_u8 fltr_buf[] = {0x01, 0x10, 0x30, 0x00, 0x06, 0x00, 0x00, 0x00, 0x01, 0xff, 0x01, 0x02, 0x00,
                       0x00, 0x00, 0x00, 0x01, 0x41, 0x01, 0x00, 0x00, 0x00, 0x01, 0x08, 0x06, 0x02,
                       0x02, 0x14, 0x00, 0x00, 0x00, 0x01, 0x41, 0x44, 0x01, 0x00, 0x00, 0x00, 0x01,
                       0xc0, 0xa8, 0x00, 0x68, 0x04, 0x02, 0x2e, 0x00, 0x00, 0x00, 0x01, 0x41, 0x44};

    ENTER();

    ret = pcb->moal_malloc(pmadapter->pmoal_handle, sizeof(mlan_ds_misc_cmd), MLAN_MEM_DEF, (t_u8 **)&hostcmd);
    ...
    else
    {
        ...
        mefcmd->criteria = wlan_cpu_to_le32(pmpriv->adapter->hs_cfg.conditions & ~MBIT(2));
    ...
    /* Send command to firmware */
    ret = wlan_prepare_cmd(pmpriv, 0, 0, 0, (t_void *)pioctl_req, (t_void *)hostcmd);
    if (ret == MLAN_STATUS_SUCCESS)
        ret = MLAN_STATUS_PENDING;
 ...
 *  @return             MLAN_STATUS_PENDING --success, otherwise fail
 */
mlan_status wlan_misc_ioctl_mef_cfg(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret               = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv          = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_misc_mef_cfg *mef_cfg = &((mlan_ds_misc_cfg *)pioctl_req->pbuf)->param.mef_cfg;
    pmlan_callbacks pcb           = &pmadapter->callbacks;
    HostCmd_DS_GEN *hostcmd_hdr;
    HostCmd_DS_MEF_CFG *mefcmd;
    ...
    t_u32 buf_len             = 0;
    t_u8 *buf, *filter;
    t_u8 fltr_buf[] = {0x02, 0x00, 0x2f, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x01, 0x00, 0x5e, 0x03,
                       0x02, 0x00, 0x00, 0x00, 0x00, 0x01, 0x41, 0x06, 0x00, 0x00, 0x00, 0x01, 0xff,
                       0x01, 0x02, 0x00, 0x00, 0x00, 0x00, 0x01, 0x41, 0x45, 0x01, 0x00, 0x00, 0x00,
                       0x01, 0x33, 0x33, 0x02, 0x02, 0x00, 0x00, 0x00, 0x00, 0x01, 0x41, 0x45};
 ...
 *  @return             MLAN_STATUS_PENDING --success, otherwise fail
 */
mlan_status wlan_misc_ioctl_ipaddr_cfg(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv   = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_misc_cfg *misc = (mlan_ds_misc_cfg *)pioctl_req->pbuf;
    mlan_status ret        = MLAN_STATUS_SUCCESS;
 ...
 *  @return             MLAN_STATUS_PENDING --success, otherwise fail
 */
mlan_status wlan_misc_ioctl_cfp_code_cfg(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret                 = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv            = pmadapter->priv[pioctl_req->bss_index];
    mlan_ds_misc_cfg *misc          = (mlan_ds_misc_cfg *)pioctl_req->pbuf;
 ...
 */
mlan_status wlan_misc_ioctl_country_code(IN pmlan_adapter pmadapter, IN mlan_ioctl_req *pioctl_req)
{
    mlan_status ret                         = MLAN_STATUS_SUCCESS;
    mlan_private *pmpriv                    = pmadapter->priv[pioctl_req->bss_index];
    ...
    if (pioctl_req->action == MLAN_ACT_SET)
    {
        ...
        if (wlan_misc_country_2_cfp_table_code(pmadapter, country_code->country_code, &cfp_bg, &cfp_a))
        {
        ...
        if (cfp_bg && cfp_a && (cfp_bg == cfp_a))
            ...
            pmadapter->region_code = 0;
        if (wlan_set_regiontable(pmpriv, pmadapter->region_code, pmadapter->config_bands | pmadapter->adhoc_start_band))
        {
            pioctl_req->status_code = MLAN_ERROR_INVALID_PARAMETER;
            ret                     = MLAN_STATUS_FAILURE;
            goto done;
 ...
 *  @return		MLAN_STATUS_SUCCESS --success
 */
static mlan_status wlan_set_get_scan_cfg(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req, IN t_u32 action)
{
    ...
    else
    {
        ...
        if (scan->param.scan_cfg.scan_time.specific_scan_time)
            pmadapter->specific_scan_time = (t_u16)scan->param.scan_cfg.scan_time.specific_scan_time;
 ...
 *  @return		MLAN_STATUS_SUCCESS/MLAN_STATUS_PENDING --success, otherwise fail
 */
static mlan_status wlan_scan_ioctl(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    pmlan_private pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_status status   = MLAN_STATUS_SUCCESS;
    mlan_ds_scan *pscan;
    ...
    if (pscan->sub_command == MLAN_OID_SCAN_CONFIG)
        goto start_config;
    if (pmadapter->scan_processing && pioctl_req->action == MLAN_ACT_SET && pscan->sub_command != MLAN_OID_SCAN_CANCEL)
    {
...
start_config:
    ...
    if (pioctl_req->action == MLAN_ACT_SET)
    {
        switch (pscan->sub_command)
        {
            ...
            case MLAN_OID_SCAN_SPECIFIC_SSID:
                status = wlan_scan_specific_ssid(pmpriv, pioctl_req, &pscan->param.scan_req.scan_ssid);
                break;
            case MLAN_OID_SCAN_USER_CONFIG:
                status =
                    wlan_scan_networks(pmpriv, pioctl_req, (wlan_user_scan_cfg *)pscan->param.user_scan.scan_cfg_buf);
                break;
            ...
            case MLAN_OID_SCAN_BGSCAN_CONFIG:
                /* Send request to firmware */
                status = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_BG_SCAN_CONFIG, HostCmd_ACT_GEN_SET, 0,
                                          (t_void *)pioctl_req, pscan->param.user_scan.scan_cfg_buf);
                break;
            default:
                pioctl_req->status_code = MLAN_ERROR_IOCTL_INVALID;
                status                  = MLAN_STATUS_FAILURE;
        ...
        if ((status == MLAN_STATUS_SUCCESS) && (pscan->sub_command != MLAN_OID_SCAN_TABLE_FLUSH) &&
            (pscan->sub_command != MLAN_OID_SCAN_CANCEL) && (pscan->sub_command != MLAN_OID_SCAN_CONFIG))
        {
    ...
    else
    {
        ...
        else if (pscan->sub_command == MLAN_OID_SCAN_GET_CURRENT_BSS)
        {
            ...
            pscan->param.scan_resp.pscan_table       = (t_u8 *)&pmpriv->curr_bss_params.bss_descriptor;
            pioctl_req->data_read_written            = sizeof(mlan_scan_resp) + MLAN_SUB_COMMAND_SIZE;
        }
        else
        {
...
mlan_status wlan_set_ewpa_mode(mlan_private *priv, mlan_ds_passphrase *psec_pp)
{
    ...
    if ((psec_pp->psk_type == MLAN_PSK_PASSPHRASE && psec_pp->psk.passphrase.passphrase_len > 0) ||
...
mlan_status wlan_find_bss(mlan_private *pmpriv, pmlan_ioctl_req pioctl_req)
{
    ...
    {
        i = wlan_find_ssid_in_list(pmpriv, &bss->param.ssid_bssid.ssid, MNULL, pmpriv->bss_mode);
        if (i < 0)
        {
