...
static raListTbl *wlan_wmm_allocate_ralist_node(pmlan_adapter pmadapter, t_u8 *ra)
{
    ...
    (void)memcpy(pmadapter, ra_list->ra, ra, MLAN_MAC_ADDR_LENGTH);
...
static void wlan_send_wmmac_host_event(pmlan_private priv, char *typeStr, t_u8 *srcAddr, t_u8 tid, t_u8 up, t_u8 status)
{
    ...
    (void)memcpy(priv->adapter, pOutBuf, (t_u8 *)"TSPEC:", 6);
    pOutBuf += 6;

    (void)memcpy(priv->adapter, pOutBuf, (t_u8 *)typeStr, wlan_strlen(typeStr));
    ...
    *pOutBuf++ = status + '0';
    ...
    (void)memcpy(priv->adapter, pOutBuf, (t_u8 *)"]:TID", 5);
    pOutBuf += 5;
    *pOutBuf++ = tid + '0';

    (void)memcpy(priv->adapter, pOutBuf, (t_u8 *)":UP", 3);
    pOutBuf += 3;
    *pOutBuf++ = up + '0';
...
int wlan_ralist_update(mlan_private *priv, t_u8 *old_ra, t_u8 *new_ra)
{
    ...
    for (tid = 0; tid < MAX_NUM_TID; ++tid)
    {
        ...
        if (ra_list)
        {
            ...
            (void)memcpy(priv->adapter, ra_list->ra, new_ra, MLAN_MAC_ADDR_LENGTH);
        }
    }
...
t_void wlan_wmm_add_buf_txqueue(pmlan_adapter pmadapter, pmlan_buffer pmbuf)
{
    ...
    else
    {
        ...
        (void)memcpy(pmadapter, ra, pmbuf->pdesc, MLAN_MAC_ADDR_LENGTH);
        /** put multicast/broadcast packet in the same ralist */
        if (ra[0] & 0x01)
            (void)memset(pmadapter, ra, 0xff, sizeof(ra));
    ...
    while (resp_len >= sizeof(pTlvHdr->header))
    {
        ...
        switch (wlan_le16_to_cpu(pTlvHdr->header.type))
        {
            ...
            case WMM_IE:
                ...
                (void)memcpy(priv->adapter, (t_u8 *)&priv->curr_bss_params.bss_descriptor.wmm_ie, pWmmParamIe,
...
mlan_status wlan_cmd_wmm_addts_req(IN pmlan_private pmpriv, OUT HostCmd_DS_COMMAND *cmd, IN t_void *pdata_buf)
{
    ...
    (void)memcpy(pmpriv->adapter, pcmd_addts->tspec_data, paddts->ie_data, MIN(WMM_TSPEC_SIZE, paddts->ie_data_len));
                                   ...
                                   OUT mlan_ioctl_req *pioctl_buf)
{
    ...
    if (pioctl_buf)
    {
        ...
        if (presp_addts->command_result == MLAN_CMD_RESULT_SUCCESS)
        {
            ...
            (void)memcpy(pmpriv->adapter, paddts->ie_data, presp_addts->tspec_data, paddts->ie_data_len);
        }
        else
        {
...
mlan_status wlan_cmd_wmm_delts_req(IN pmlan_private pmpriv, OUT HostCmd_DS_COMMAND *cmd, IN t_void *pdata_buf)
{
    ...
    (void)memcpy(pmpriv->adapter, pcmd_delts->tspec_data, pdelts->ie_data, MIN(WMM_TSPEC_SIZE, pdelts->ie_data_len));
...
mlan_status wlan_cmd_wmm_ts_status(IN pmlan_private pmpriv, OUT HostCmd_DS_COMMAND *cmd, IN t_void *pdata_buf)
{
    ...
    (void)memcpy(pmpriv->adapter, (t_void *)pcmd_ts_status, (t_void *)pts_status, sizeof(HostCmd_DS_WMM_TS_STATUS));
...
mlan_status wlan_ret_wmm_ts_status(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *resp, OUT mlan_ioctl_req *pioctl_buf)
{
    ...
    if (pioctl_buf)
    {
        ...
        (void)memcpy(pmpriv->adapter, (t_void *)&pwmm->param.ts_status, (t_void *)presp_ts_status,
