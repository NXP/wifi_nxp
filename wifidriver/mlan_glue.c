...
int mlan_subsys_init(void)
{
    ...
    mlan_dev.bss_attr[2].bss_type = MLAN_BSS_TYPE_WIFIDIRECT;
    /** Data frame type: Ethernet II, 802.11, etc. */
    mlan_dev.bss_attr[2].frame_type = MLAN_DATA_FRAME_TYPE_ETH_II;
    /** The BSS is active (non-0) or not (0). */
    mlan_dev.bss_attr[2].active = MTRUE;
    /** BSS Priority */
    mlan_dev.bss_attr[2].bss_priority = 0;
    /** BSS number */
    mlan_dev.bss_attr[2].bss_num = 0;
#endif
...
int wrapper_wlan_cmd_11n_addba_rspgen(void *saved_event_buff)
{
    ...
    {
        cmd->seq_num = wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[evt->bss_type]);
...
{
    pmlan_private priv = mlan_adap->priv[rxpd->bss_type];
...
)
{
    ...
    mlan_private *pmpriv_uap = mlan_adap->priv[interface];
    ...
    if ((ptx_tbl = wlan_11n_get_txbastream_tbl(pmpriv_uap, addr)))
    {
            ...
            && ptx_tbl->ba_status != BA_STREAM_SETUP_INPROGRESS)
        {
            ...
            ret = wifi_event_completion((enum wlan_bss_type)interface, WIFI_EVENT_11N_SEND_ADDBA, WIFI_EVENT_REASON_SUCCESS, addba);
            if (ret != WM_SUCCESS)
            {
...
int wrapper_wlan_11d_enable(int bss_type, t_u32 state)
{
    ...
    wlan_ops_sta_prepare_cmd((mlan_private *)mlan_adap->priv[bss_type], HostCmd_CMD_802_11_SNMP_MIB, HostCmd_ACT_GEN_SET,
...
#if CONFIG_WPA_SUPP_P2P
        || ((bss_type == MLAN_BSS_TYPE_WIFIDIRECT) && (mlan_adap->priv[bss_type]->bss_role == MLAN_BSS_ROLE_UAP))
#endif
	    )
        status = wlan_ops_uap_prepare_cmd((mlan_private *)mlan_adap->priv[bss_type], HOST_CMD_APCMD_SYS_CONFIGURE, action, 0,
                                          NULL, pdata_buf, cmd);
    else
#endif
        status = wlan_ops_sta_prepare_cmd((mlan_private *)mlan_adap->priv[bss_type], HostCmd_CMD_MGMT_IE_LIST, action, 0, NULL,
                                          pdata_buf, cmd);
...
#if CONFIG_WPA_SUPP_P2P
    req.bss_index = bss->bss_type;
    req.action    = bss->action;
#else
    req.bss_index = 0;
#endif
    ...
    unsigned int owe_trans_mode, bool is_ft, int key_mgmt)
{
    ...
    if (wlan_11d_support_is_enabled(priv) && !mlan_adap->country_ie_ignore)
    {
        ...
        {
            if (wifi_create_dnld_countryinfo() != MLAN_STATUS_SUCCESS)
            {
                return -WM_FAIL;
            }
...
#endif
#if CONFIG_WPA_SUPP_P2P
        priv->p2p.session_enable = MFALSE;
#endif
...
#endif
#if CONFIG_WPA_SUPP_P2P
            if ((pvendor_ie->element_id == P2P_IE) &&
                (!__memcmp(priv->adapter, pvendor_ie->oui, p2p_oui, sizeof(pvendor_ie->oui))) &&
                (pvendor_ie->oui_type == p2p_oui[3U]))
        {
            /* Test to see if it is a P2P IE*/
            priv->p2p.session_enable = MTRUE;
            __memcpy(priv->adapter, (t_u8 *)&priv->p2p.p2p_ie, ie_data_ptr, ie_len);
            PRINTM(MINFO, "P2P_IE added.\n");
        }
        else
#endif
            {
...
int wifi_set_scan_ies(void *ie, size_t ie_len)
{
    ...
    {
        priv = (mlan_private *)mlan_adap->priv[2];
    }
    priv->p2p.session_enable = MFALSE;
#endif
...
int wifi_assocreq_p2p_ie_cfg(mlan_private *priv)
{
    ...
    p2pie_len = sizeof(IEEEtypes_Header_t) + priv->p2p.p2p_ie.vend_hdr.len;
...
{
    mlan_ds_bss bss;
    t_u8 bss_mode        = BSS_MODE_WIFIDIRECT_GO;
    mlan_private *pmpriv = (mlan_private *)mlan_adap->priv[bss_type];

    (void)memset(&bss, 0x00, sizeof(mlan_ds_bss));
    bss.sub_command = MLAN_OID_WIFI_DIRECT_MODE;
    bss.bss_type    = bss_type;
    bss.action      = MLAN_ACT_SET;

    bss.param.wfd_mode = WIFI_DIRECT_MODE_NONE; /*Mode 0*/
    wifi_send_bss_ioctl(&bss);

    bss.param.wfd_mode = WIFI_DIRECT_MODE_DEVICE; /*Mode 1*/
    wifi_send_bss_ioctl(&bss);
...
int wifi_nxp_send_assoc(unsigned int bss_type, nxp_wifi_assoc_info_t *assoc_info)
{
    ...
    {
        priv->p2p_gc_network = true;
        priv->bss_role       = MLAN_BSS_ROLE_STA;
        wifi_set_wifi_direct_mode(bss_type, WIFI_DIRECT_MODE_CLIENT);
    }
#endif
    ...
    else
    {
        switch (command)
        {
            ...
            case HOST_CMD_APCMD_BSS_STOP:
            {
                if (resp->result == HostCmd_RESULT_OK)
                {
			    ...
			    )
                    {
                        ...
                        (void)wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_UAP_STOPPED, WIFI_EVENT_REASON_SUCCESS, NULL);
                    }
                }
                else
                {
...
#if CONFIG_WPA_SUPP
#if CONFIG_HOSTAPD
                            wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_ACS_COMPLETE, WIFI_EVENT_REASON_SUCCESS, NULL);
#endif
#endif
                        }
...
#endif
                        wm_wifi.cmd_resp_status = WM_SUCCESS;
                        (void)wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_UAP_STARTED, WIFI_EVENT_REASON_SUCCESS, NULL);
                    }
                }
                else
                {
            ...
            case HostCmd_CMD_GET_HW_SPEC:
                ...
                (void)wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_GET_HW_SPEC, WIFI_EVENT_REASON_SUCCESS, NULL);
                break;
            case HostCmd_CMD_802_11_SCAN:
                if (resp->result != HostCmd_RESULT_OK)
                {
                    ...
                    (void)wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_SCAN_RESULT, WIFI_EVENT_REASON_FAILURE, NULL);
                    break;
                }
                ...
                if (is_split_scan_complete())
                {
                    ...
                    (void)wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_SCAN_RESULT, WIFI_EVENT_REASON_SUCCESS, NULL);
                }
                break;
...
#if !CONFIG_WPA_SUPP
                (void)wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_DEAUTHENTICATION, WIFI_EVENT_REASON_SUCCESS, NULL);
#endif
                break;
            ...
            case HostCmd_CMD_802_11_PS_MODE_ENH:
            {
                ...
                if (ps_action_p != NULL)
                {
                    ...
                    if (ps_event != (t_u16)WIFI_EVENT_PS_INVALID)
                    {
                        ...
                        {
                            if (wifi_event_completion((enum wlan_bss_type)bss_type, (enum wifi_event)ps_event, result,
...
#endif
                        {
                            if (wifi_event_completion((enum wlan_bss_type)bss_type, (enum wifi_event)ps_event, result, (void *)ps_action_p) !=
                                WM_SUCCESS)
                            {
#if !CONFIG_MEM_POOLS
                                OSA_MemoryFree((void *)ps_action_p);
            ...
            case HostCmd_CMD_SUPPLICANT_PMK:
                (void)wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_SUPPLICANT_PMK,
            ...
            case HostCmd_CMD_802_11_ASSOCIATE:
            {
                ...
                if (rv != MLAN_STATUS_SUCCESS)
                {
                    (void)wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_ASSOCIATION, WIFI_EVENT_REASON_FAILURE, NULL);
                    return -WM_FAIL;
                }
#endif
            ...
            assoc_resp_ret:
#endif
                (void)wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_ASSOCIATION, result, NULL);
            }
            break;
            case HostCmd_CMD_802_11_MAC_ADDRESS:
            {
                ...
                if (bss_type == MLAN_BSS_TYPE_STA)
                {
                    ...
                    ret = wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_STA_MAC_ADDR_CONFIG, WIFI_EVENT_REASON_SUCCESS, sta_addr);
                }
                else if (bss_type == MLAN_BSS_TYPE_UAP)
                {
                    (void)memcpy(dev_mac_addr_uap, sta_addr, MLAN_MAC_ADDR_LENGTH);
                    ret = wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_UAP_MAC_ADDR_CONFIG, WIFI_EVENT_REASON_SUCCESS, sta_addr);
                }
                else if (bss_type == MLAN_BSS_TYPE_WIFIDIRECT)
                {
                    (void)memcpy(dev_mac_addr_wfd, sta_addr, MLAN_MAC_ADDR_LENGTH);
                    ret = wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_WFD_MAC_ADDR_CONFIG, WIFI_EVENT_REASON_SUCCESS, sta_addr);
                }
            ...
            case HostCmd_CMD_TWT_CFG:
            {
                if (resp->result == HostCmd_RESULT_OK)
                {
                    if (wm_wifi.cmd_resp_priv != NULL)
                    {
                        ...
                        {
                            mlan_ds_twt_report *rpt = (mlan_ds_twt_report *)wm_wifi.cmd_resp_priv;
                            (void)memcpy(rpt, &resp->params.twtcfg.param.twt_report, sizeof(mlan_ds_twt_report));
                        }
                        else if (resp->params.twtcfg.sub_id == MLAN_11AX_TWT_BTWT_SUBID)
                        {
                            mlan_ds_btwt_cfg *cfg = (mlan_ds_btwt_cfg *)wm_wifi.cmd_resp_priv;
                            (void)memcpy(cfg, &resp->params.twtcfg.param.btwt_cfg, sizeof(mlan_ds_btwt_cfg));
                        }
                    }
                    wm_wifi.cmd_resp_status = WM_SUCCESS;
                }
            ...
            case HostCmd_CMD_802_11_REMAIN_ON_CHANNEL:
            {
                ...
                if (remain_channel_info != NULL)
                {
                    ...
                    {
                        if (wifi_event_completion((enum wlan_bss_type)bss_type, WIFI_EVENT_REMAIN_ON_CHANNEL, WIFI_EVENT_REASON_SUCCESS,
...
//    }
    ...
    while (tlv_buf_left >= (int)sizeof(MrvlIEtypesHeader_t))
    {
        ...
        if (tlv_type == TLV_TYPE_TX_PAUSE)
        {
...
#if CONFIG_WPA_SUPP_P2P
                || ((evt->bss_type == MLAN_BSS_TYPE_WIFIDIRECT) &&
                    (mlan_adap->priv[evt->bss_type]->bss_role == MLAN_BSS_ROLE_STA))
#endif
		    )
            {
                if (wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_TX_DATA_PAUSE, WIFI_EVENT_REASON_SUCCESS, tx_pause_tlv) !=
                    WM_SUCCESS)
                {
#if !CONFIG_MEM_POOLS
            ...
            {
                if (wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_UAP_TX_DATA_PAUSE, WIFI_EVENT_REASON_SUCCESS, tx_pause_tlv) !=
                    WM_SUCCESS)
                {
#if !CONFIG_MEM_POOLS
                    OSA_MemoryFree(tx_pause_tlv);
...
int wifi_handle_fw_event(struct bus_message *msg)
{
    ...
    if (evt->bss_type == BSS_TYPE_WFD)
    {
        pmpriv_uap = pmpriv = (mlan_private *)mlan_adap->priv[2];
    }
#endif
    ...
    switch (evt->event_id)
    {
        ...
        case EVENT_DEAUTHENTICATED:
            ...
            {
                (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_AUTHENTICATION, WIFI_EVENT_REASON_FAILURE,
...
#if !CONFIG_WPA_SUPP
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_DISASSOCIATION, WIFI_EVENT_REASON_FAILURE,
                                        (void *)IEEEtypes_REASON_DEAUTH_LEAVING);
#else
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_DISASSOCIATION, WIFI_EVENT_REASON_SUCCESS,
                                        (void *)&evt->reason_code);
#endif
...
#if !CONFIG_WPA_SUPP
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_AUTHENTICATION, WIFI_EVENT_REASON_SUCCESS, NULL);
#endif
            break;
        case EVENT_PS_SLEEP:
#if CONFIG_WIFI_PS_DEBUG
        ...
        case EVENT_WNM_PS:
        {
            ...
            *wnm_action_p = evt->reason_code;
            ...
            if (wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_WNM_PS, WIFI_EVENT_REASON_SUCCESS, (void *)((t_u32)wnm_action_p)) !=
                WM_SUCCESS)
            {
                /* If fail to send message on queue, free allocated memory ! */
#if !CONFIG_MEM_POOLS
        ...
        case EVENT_MIC_ERR_MULTICAST:
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_ERR_MULTICAST, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
        case EVENT_MIC_ERR_UNICAST:
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_ERR_UNICAST, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
#if CONFIG_BG_SCAN
        case EVENT_BG_SCAN_REPORT:
            pmpriv->adapter->bgscan_reported = MTRUE;
            pmpriv->roaming_configured       = MFALSE;
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_BG_SCAN_REPORT, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
        case EVENT_BG_SCAN_STOPPED:
            pmpriv->roaming_configured = MFALSE;
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_BG_SCAN_STOPPED, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
#endif
#if CONFIG_HOST_SLEEP
        case EVENT_HS_ACT_REQ:
            if (pmpriv->adapter->is_hs_configured == MTRUE)
                (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_HS_CONFIG, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
#endif
        ...
        case EVENT_SNR_HIGH:
            ...
            break;
        case EVENT_MAX_FAIL:
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_MAX_FAIL, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
        case EVENT_DATA_RSSI_LOW:
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_DATA_RSSI_LOW, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
        case EVENT_DATA_RSSI_HIGH:
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_DATA_RSSI_HIGH, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
        case EVENT_DATA_SNR_LOW:
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_DATA_SNR_LOW, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
        case EVENT_DATA_SNR_HIGH:
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_DATA_SNR_HIGH, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
        case EVENT_LINK_QUALITY:
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_FW_LINK_QUALITY, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
        case EVENT_PRE_BEACON_LOST:
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_FW_PRE_BCN_LOST, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
#endif
#if CONFIG_11N
        case EVENT_ADDBA:
        {
            ...
            {
                if (wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_11N_RECV_ADDBA, WIFI_EVENT_REASON_SUCCESS, saved_event_buff) !=
                    WM_SUCCESS)
                {
                    /* If fail to send message on queue, free allocated memory ! */
#if !CONFIG_MEM_POOLS
        ...
        case EVENT_BA_STREAM_TIMEOUT:
        {
            ...
            if (saved_event_buff != NULL)
            {
                if (wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_11N_BA_STREAM_TIMEOUT, WIFI_EVENT_REASON_SUCCESS,
        ...
        case EVENT_AMSDU_AGGR_CTRL:
            ...
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_11N_AGGR_CTRL, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
#endif /* CONFIG_11N */
        case EVENT_CHANNEL_SWITCH_ANN:
            (void)wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_CHAN_SWITCH_ANN, WIFI_EVENT_REASON_SUCCESS, NULL);
            break;
        case EVENT_CHANNEL_SWITCH:
        {
            ...
            if (wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_CHAN_SWITCH, WIFI_EVENT_REASON_SUCCESS, pecsa_info) != WM_SUCCESS)
            {
                /* If fail to send message on queue, free allocated memory ! */
#if !CONFIG_MEM_POOLS
        ...
        case EVENT_MICRO_AP_STA_ASSOC:
        {
...
#if (CONFIG_UAP_AMPDU_TX) || (CONFIG_UAP_AMPDU_RX)
            wlan_request_ralist_lock(mlan_adap->priv[evt->bss_type]);
            /* Clear corresponding tx/rx table if necessary */
            if (wlan_11n_get_txbastream_tbl((mlan_private *)mlan_adap->priv[evt->bss_type], sta_addr))
                wlan_11n_delete_txbastream_tbl_entry((mlan_private *)mlan_adap->priv[evt->bss_type], sta_addr);

            wlan_cleanup_reorder_tbl((mlan_private *)mlan_adap->priv[evt->bss_type], sta_addr);
#if CONFIG_WMM
            wlan_ralist_del_enh(mlan_adap->priv[evt->bss_type], sta_addr);
#endif
            /* txbastream table also is used as connected STAs data base */
            wlan_11n_create_txbastream_tbl((mlan_private *)mlan_adap->priv[evt->bss_type], sta_addr, BA_STREAM_NOT_SETUP);
            wlan_11n_update_txbastream_tbl_tx_thresh((mlan_private *)mlan_adap->priv[evt->bss_type], sta_addr, 3);
            ...
            wrapper_wlan_check_sta_capability((mlan_private *)mlan_adap->priv[evt->bss_type], msg->data, sta_node_ptr);

            if (sta_node_ptr->is_11n_enabled)
            {
                wlan_11n_update_txbastream_tbl_ampdu_supported((mlan_private *)mlan_adap->priv[evt->bss_type], sta_addr, MTRUE);
            }

            wlan_release_ralist_lock(mlan_adap->priv[evt->bss_type]);
            ...
            if (wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_UAP_CLIENT_ASSOC, WIFI_EVENT_REASON_SUCCESS, sta_addr) != WM_SUCCESS)
            {
                /* If fail to send message on queue, free allocated memory ! */
#if !CONFIG_MEM_POOLS
                OSA_MemoryFree((void *)sta_addr);
        ...
        case EVENT_MICRO_AP_RSN_CONNECT:
        {
            ...
            event_sta_addr = (t_u8 *)&evt->src_mac_addr;
            (void)memcpy((void *)sta_addr, (const void *)event_sta_addr, MLAN_MAC_ADDR_LENGTH);
            if (wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_UAP_CLIENT_CONN, WIFI_EVENT_REASON_SUCCESS, sta_addr) != WM_SUCCESS)
            {
                /* If fail to send message on queue, free allocated memory ! */
#if !CONFIG_MEM_POOLS
                OSA_MemoryFree((void *)sta_addr);
        ...
        case EVENT_MICRO_AP_STA_DEAUTH:
            ...
            if (wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_UAP_CLIENT_DEAUTH, WIFI_EVENT_REASON_SUCCESS, disassoc_resp) !=
                WM_SUCCESS)
            {
                /* If fail to send message on queue, free allocated memory ! */
#if !CONFIG_MEM_POOLS
...
#if CONFIG_WMM
            wlan_ralist_del_enh(mlan_adap->priv[evt->bss_type], evt->src_mac_addr);
#endif
        ...
        case EVENT_MICRO_AP_BSS_START:
             ...
             */
#if (CONFIG_UAP_AMPDU_TX) || (CONFIG_UAP_AMPDU_RX)
            wrapper_wlan_check_uap_capability((mlan_private *)mlan_adap->priv[evt->bss_type], msg->data);
#endif /* CONFIG_UAP_AMPDU_TX || CONFIG_UAP_AMPDU_RX */
        ...
        case EVENT_EXT_SCAN_REPORT:
            ...
            if (is_split_scan_complete() && !pext_scan_result->more_event)
            {
                wifi_d("Split scan complete");
                wifi_user_scan_config_cleanup();
                wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_SCAN_RESULT, WIFI_EVENT_REASON_SUCCESS, NULL);
            }
#endif
            break;
#endif
        ...
        case EVENT_CSI:
        {
            ...
            if(!mlan_adap->ami_ongoing)
            {
                ...
                wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_CSI_PROC, WIFI_EVENT_REASON_SUCCESS, csi_proc_data);
            }
#endif
#endif
            ...
            if (g_csi_event_for_wls)
            {
                ...
                wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_WLS_CSI, WIFI_EVENT_REASON_SUCCESS, wls_data);
            }
#endif
#endif
        }
        ...
        case EVENT_CSI_STATUS:
        {
            ...
            if (wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_CSI_STATUS, WIFI_EVENT_REASON_SUCCESS, pcsi_status) != WM_SUCCESS)
            {
                /* If fail to send message on queue, free allocated memory ! */
#if !CONFIG_MEM_POOLS
        ...
        case EVENT_ASSOC_REQ_IE:
            ...
            if (pmpriv->media_connected == MTRUE)
            {
                wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_ASSOCIATION_NOTIFY, WIFI_EVENT_REASON_SUCCESS, NULL);
            }
#endif
        ...
        case EVENT_CHAN_LOAD:
        {
            ...
            if(wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_CHAN_LOAD, WIFI_EVENT_REASON_SUCCESS, chan_load) != WM_SUCCESS)
            {
                /* If fail to send message on queue, free allocated memory ! */
#if !CONFIG_MEM_POOLS
        ...
        case EVENT_REMAIN_ON_CHANNEL_EXPIRED:
            if (wifi_is_remain_on_channel() == MTRUE)
            {
                ...
                if (remain_channel_info != NULL)
                {
                    ...
                    if (wifi_event_completion((enum wlan_bss_type)evt->bss_type, WIFI_EVENT_REMAIN_ON_CHANNEL, WIFI_EVENT_REASON_SUCCESS,
                        (void *)remain_channel_info) != WM_SUCCESS)
                    {
#if !CONFIG_MEM_POOLS
                        OSA_MemoryFree(remain_channel_info);
...
int wifi_set_btwt_cfg(const wifi_btwt_config_t *btwt_cfg)
{
    ...
    (void)memcpy(&twt_cfg.param.btwt_cfg, btwt_cfg, sizeof(twt_cfg.param.btwt_cfg));

    if (twt_cfg.param.btwt_cfg.count < 2)
    {
        wifi_put_command_lock();
        wifi_e("minimum broadcast TWT sessions is two");
        return -WM_FAIL;
    }
...
int wifi_get_mc_cfg(uint32_t *channel_time)
{
    ...
    mlan_ds_misc_cfg *ioctl_cfg = NULL;

    (void)memset(cmd, 0x00, sizeof(HostCmd_DS_COMMAND));
#if !CONFIG_MEM_POOLS
    ioctl_cfg = OSA_MemoryAllocate(sizeof(mlan_ds_misc_cfg));
#else
    ioctl_cfg = OSA_MemoryPoolAllocate(buf_3072_MemoryPool);
#endif
    if (ioctl_cfg == NULL)
    {
        wifi_put_command_lock();
 ...
 *
 *  @param data  Data written to flash
 *  @param length  Data length
 *  @param write_addr  Current written address in flash
...
int wifi_flash_dump_append_data(t_u8 *data, t_u32 length, t_u32 *write_addr, bool end)
{
    ...
    if (fw_dump_page_buf_cached_len > 0 && (fw_dump_page_buf_cached_len + length >= MFLASH_PAGE_SIZE)) {
        buf = OSA_MemoryAllocate(fw_dump_page_buf_cached_len + length);
        if (!buf)
        {
            wifi_e("Failed to allocate buffer for FW dump");
            return -WM_FAIL;
        }
        memcpy(buf, fw_dump_page_buf_cached, fw_dump_page_buf_cached_len);
        memcpy(buf + fw_dump_page_buf_cached_len, data, length);
        ptr = buf;
        length += fw_dump_page_buf_cached_len;
        fw_dump_page_buf_cached_len = 0;
    }

    while (length >= MFLASH_PAGE_SIZE) {
        memcpy(fw_dump_page_buffer, ptr, MFLASH_PAGE_SIZE);

        status = mflash_drv_page_program(mflash_drv_log2phys((void *)(*write_addr), MFLASH_SECTOR_SIZE),
                                             (uint32_t *)fw_dump_page_buffer);
        if (status != WM_SUCCESS) {
            goto done;
        }

        *write_addr += MFLASH_PAGE_SIZE;
        ptr += MFLASH_PAGE_SIZE;
        length -= MFLASH_PAGE_SIZE;
    }

    if (length > 0) {
        if (end) {
            memset(fw_dump_page_buffer, 0xFF, MFLASH_PAGE_SIZE);
            memcpy(fw_dump_page_buffer, ptr, length);
            ...
            *write_addr += length;
            ...
            memcpy(fw_dump_page_buf_cached + fw_dump_page_buf_cached_len, ptr, length);
            fw_dump_page_buf_cached_len += length;
        }
    }
...
void wifi_dump_firmware_info()
{
    ...
    else if (!fw_dump_sector_erased)
    {
        (void)mflash_drv_init();
        (void)PRINTF("Start erase flash\r\n");
        t_u32 erase_start = CONFIG_FW_DUMP_FLASH_START_ADDR & ~(MFLASH_SECTOR_SIZE - 1);
        t_u32 erase_end = (CONFIG_FW_DUMP_FLASH_START_ADDR + CONFIG_FW_DUMP_FLASH_ERASE_LENGTH +
                           MFLASH_SECTOR_SIZE - 1) & ~(MFLASH_SECTOR_SIZE - 1);
        for (t_u32 erase_addr = erase_start; erase_addr < erase_end; erase_addr += MFLASH_SECTOR_SIZE) {
            int status = mflash_drv_sector_erase(mflash_drv_log2phys((void *)erase_addr, MFLASH_SECTOR_SIZE));
            if (status != kStatus_Success) {
                wifi_e("Erase flash: %p failed:%u", erase_addr, status);
    ...
    do
    {
        ...
        {
            ret = wifi_flash_dump_append_data(data, sizeof(data), &fw_dump_current_addr,
                                              (DEBUG_SQRAM_DONE == ctrl_data ? 1 : 0));
            if (ret != WM_SUCCESS)
            {
                wifi_e("Flash writing failed");
                goto done;
...
#ifdef RW610
#define DEBUG_DUMP_CTRL_REG     (uint32_t *)0x41382488
#define DEBUG_DUMP_START_REG    (uint32_t *)0x41382494
#define DEBUG_DUMP_DATA_LENGTH  8U
#define MAX_POLL_TRIES          100U
/**
 *  @brief This function read/write firmware via SMU
 *
 *  @param doneflag  A flag
 *
 *  @return         MLAN_STATUS_SUCCESS
 */
rdwr_status wifi_smu_rdwr_firmware(t_u8 doneflag)
{
    int tries              = 0;
    t_u8 ctrl_data         = 0;
    ...
    *DEBUG_DUMP_CTRL_REG = debug_host_ready;
    for (tries = 0; tries < MAX_POLL_TRIES; tries++)
    {
        ctrl_data = (*DEBUG_DUMP_CTRL_REG) & 0xff;
...
/**
 *  @brief This function dump firmware memory to file or flash
 *
 *  @return         N/A
 */
void wifi_dump_firmware_info()
{
...
#ifdef RW610
    t_u8 data[DEBUG_DUMP_DATA_LENGTH] = {0};
    t_u8 start_flag[DEBUG_DUMP_DATA_LENGTH] = {0};
#else
    unsigned int reg, reg_start, reg_end;
    t_u8 start_flag = 0;
...
#endif
    {
        for (tries = 0; tries < MAX_POLL_TRIES; tries++)
        {
            ...
            if (memcmp(start_flag, (void *)DEBUG_DUMP_START_REG, DEBUG_DUMP_DATA_LENGTH) == 0)
            {
                break;
            }
#else
    ...
    PRINTF("Start %s output, time %u us, please wait...\r\n", pmem_type_mapping_tbl->mem_name, OSA_GetTimestamp());
    do
    {
#ifdef RW610
        stat = wifi_smu_rdwr_firmware(doneflag);
