...
t_void wlan_reset_connect_state(pmlan_private priv, t_u8 drv_disconnect)
{
    ...
    if ((wlan_11d_is_enabled(priv)) && (priv->state_11d.user_enable_11d == DISABLE_11D))
    {
        ...
        ret = wlan_prepare_cmd(priv, HostCmd_CMD_802_11_SNMP_MIB, HostCmd_ACT_GEN_SET, Dot11D_i, MNULL, &enable);
        if (ret)
            PRINTM(MERROR, "11D: Failed to enable 11D\n");
    }
    if (pmadapter->num_cmd_timeout && pmadapter->curr_cmd && (pmadapter->cmd_timer_is_set == MFALSE))
    {
        LEAVE();
        return;
    }
...
mlan_status wlan_ops_sta_process_event(IN t_void *priv)
{
    ...
    switch (eventcause)
    {
        case EVENT_DUMMY_HOST_WAKEUP_SIGNAL:
            PRINTM(MERROR, "Invalid EVENT: DUMMY_HOST_WAKEUP_SIGNAL, ignoring it\n");
            break;
        ...
        case EVENT_PS_AWAKE:
            ...
            if (!pmadapter->pps_uapsd_mode && pmpriv->media_connected &&
                (pmpriv->port_open || !pmpriv->port_ctrl_mode) && pmadapter->sleep_period.period)
            {
        ...
        case EVENT_HS_ACT_REQ:
            ...
            ret = wlan_prepare_cmd(priv, HostCmd_CMD_802_11_HS_CFG_ENH, 0, 0, MNULL, MNULL);
            break;
        ...
        case EVENT_MEAS_REPORT_RDY:
            ...
            wlan_prepare_cmd(priv, HostCmd_CMD_MEASUREMENT_REPORT, HostCmd_ACT_GEN_SET, 0, 0, MNULL);
            break;
        case EVENT_WMM_STATUS_CHANGE:
            if (pmbuf && pmbuf->data_len > sizeof(eventcause) + sizeof(MrvlIEtypesHeader_t))
            {
                PRINTM(MEVENT, "EVENT: WMM status changed: %d\n", pmbuf->data_len);

                evt_buf = (pmbuf->pbuf + pmbuf->data_offset + sizeof(eventcause));

                wlan_ret_wmm_get_status(pmpriv, evt_buf, pmbuf->data_len - sizeof(eventcause));
            }
            else
            {
        ...
        case EVENT_IBSS_COALESCED:
            ...
            ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_IBSS_COALESCING_STATUS, HostCmd_ACT_GEN_GET, 0, MNULL,
                                   MNULL);
            break;
        case EVENT_ADDBA:
