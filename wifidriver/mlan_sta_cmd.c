...
static mlan_status wlan_cmd_mfg_tx_cont(pmlan_private pmpriv, HostCmd_DS_COMMAND *cmd, t_u16 action, t_void *pdata_buf)
{
    ...
    (void)memset(pmpriv->adapter, mcmd, 0x00, sizeof(HostCmd_DS_MFG_CMD_TX_CONT));
...
static mlan_status wlan_cmd_mfg_tx_frame(pmlan_private pmpriv, HostCmd_DS_COMMAND *cmd, t_u16 action, t_void *pdata_buf)
{
    ...
    (void)memset(pmpriv->adapter, mcmd, 0x00, sizeof(HostCmd_DS_MFG_CMD_TX_FRAME2));
    ...
    if (action == HostCmd_ACT_GEN_SET)
    {
        ...
        (void)memcpy(pmpriv->adapter, mcmd->bssid, cfg->bssid, MLAN_MAC_ADDR_LENGTH);
    }
...
static mlan_status wlan_cmd_mfg(pmlan_private pmpriv, HostCmd_DS_COMMAND *cmd, t_u16 action, t_void *pdata_buf)
{
    ...
    (void)memset(pmpriv->adapter, mcmd, 0x00, sizeof(HostCmd_DS_MFG_CMD_GENERIC_CFG));
                                          ...
                                          IN hs_config_param *pdata_buf)
{
    ...
    if (!hs_activate && (pdata_buf->conditions != HOST_SLEEP_CFG_CANCEL) &&
        ((pmadapter->arp_filter_size > 0) && (pmadapter->arp_filter_size <= ARP_FILTER_MAX_BUF_SIZE)))
    {
        PRINTM(MINFO, "Attach %d bytes ArpFilter to HSCfg cmd\n", pmadapter->arp_filter_size);
        (void)memcpy(pmpriv->adapter, ((t_u8 *)phs_cfg) + sizeof(HostCmd_DS_802_11_HS_CFG_ENH), pmadapter->arp_filter,
                     pmadapter->arp_filter_size);
        cmd->size =
            (t_u16)wlan_cpu_to_le16(pmadapter->arp_filter_size + sizeof(HostCmd_DS_802_11_HS_CFG_ENH) + S_DS_GEN);
    }
    else
#endif /* CONFIG_MLAN_WMSDK*/
                                               ...
                                               t_void *pdata_buf)
{
    ...
    if (cmd_action == HostCmd_ACT_GEN_SET)
    {
        (void)memcpy(pmpriv->adapter, cmd->params.mac_addr.mac_addr, pdata_buf, MLAN_MAC_ADDR_LENGTH);
 ...
 * @return             MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_cmd_802_11_sleep_params(IN pmlan_private pmpriv,
                                                IN HostCmd_DS_COMMAND *cmd,
                                                IN t_u16 cmd_action,
                                                IN t_u16 *pdata_buf)
{
    ...
    cmd->size    = wlan_cpu_to_le16(sizeof(HostCmd_DS_802_11_SLEEP_PARAMS) + S_DS_GEN);
    if (cmd_action == HostCmd_ACT_GEN_SET)
    {
                                              ...
                                              IN t_void *pdata_buf)
{
    ...
    (void)memcpy(pmpriv->adapter, pmc_addr->mac_list, pmcast_list->mac_list,
                                                        ...
                                                        IN t_void *pdata_buf)
{
    ...
    /* Set AP MAC address */
    (void)memcpy(pmpriv->adapter, pdeauth->mac_addr, (t_u8 *)pdata_buf, MLAN_MAC_ADDR_LENGTH);
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_cmd_802_11_ad_hoc_stop(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *cmd)
{
    ENTER();
 ...
 *  @return                 MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
static mlan_status wlan_set_keyparamset_wep(mlan_private *priv,
                                            ...
                                            t_u16 *key_param_len)
{
    ...
    {
        if ((priv->wep_key[i].key_length == WEP_40_BIT_LEN) || (priv->wep_key[i].key_length == WEP_104_BIT_LEN))
        {
            key_param_set->type = wlan_cpu_to_le16(TLV_TYPE_KEY_MATERIAL);
            /** Key_param_set WEP fixed length */
#define KEYPARAMSET_WEP_FIXED_LEN 8
            key_param_set->length = wlan_cpu_to_le16((t_u16)(priv->wep_key[i].key_length + KEYPARAMSET_WEP_FIXED_LEN));
            key_param_set->key_type_id = wlan_cpu_to_le16(KEY_TYPE_ID_WEP);
            key_param_set->key_info =
                wlan_cpu_to_le16(KEY_INFO_WEP_ENABLED | KEY_INFO_WEP_UNICAST | KEY_INFO_WEP_MCAST);
            key_param_set->key_len = (t_u16)wlan_cpu_to_le16(priv->wep_key[i].key_length);
            /* Set WEP key index */
 ...
 *  @return         MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
static mlan_status wlan_cmd_802_11_set_wep(mlan_private *priv, HostCmd_DS_COMMAND *cmd, t_u16 cmd_action)
{
    ...
    if (cmd_action == HostCmd_ACT_ADD)
    {
        ...
        wep->key_index = wlan_cpu_to_le16(priv->wep_key_curr_index & HostCmd_WEP_KEY_INDEX_MASK);
    ...
    else if (cmd_action == HostCmd_ACT_REMOVE)
    {
        ...
        /* default tx key index */
        wep->key_index = wlan_cpu_to_le16((t_u16)(priv->wep_key_curr_index & (t_u32)HostCmd_WEP_KEY_INDEX_MASK));
    }
    ...
    IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *cmd, IN t_u16 cmd_action, IN t_u32 cmd_oid, IN t_void *pdata_buf)
{
    ...
    if (cmd_action == HostCmd_ACT_GEN_GET)
    {
        ...
        (void)memcpy(pmpriv->adapter, pkey_material->key_param_set.mac_addr, pkey->mac_addr, MLAN_MAC_ADDR_LENGTH);
    ...
    (void)memset(pmpriv->adapter, &pkey_material->key_param_set, 0, sizeof(MrvlIEtype_KeyParamSetV2_t));
    ...
    if (pkey->key_len <= MAX_WEP_KEY_SIZE)
    {
        ...
        (void)memcpy(pmpriv->adapter, pkey_material->key_param_set.key_params.wep.key, pkey->key_material,
                     pkey->key_len);
        cmd->size = wlan_cpu_to_le16(sizeof(MrvlIEtypesHeader_t) + S_DS_GEN + KEY_PARAMS_FIXED_LEN +
    ...
    if (pkey->is_wapi_key)
    {
        ...
        (void)memcpy(pmpriv->adapter, pkey_material->key_param_set.key_params.wapi.pn, pkey->pn, PN_SIZE);
        pkey_material->key_param_set.key_params.wapi.key_len = wlan_cpu_to_le16(pkey->key_len);
        (void)memcpy(pmpriv->adapter, pkey_material->key_param_set.key_params.wapi.key, pkey->key_material,
                     pkey->key_len);
    ...
    if (pkey->key_len == WPA_AES_KEY_LEN)
    {
        ...
        if (pkey->key_flags & (KEY_FLAG_RX_SEQ_VALID | KEY_FLAG_TX_SEQ_VALID))
            (void)memcpy(pmpriv->adapter, pkey_material->key_param_set.key_params.aes.pn, pkey->pn, SEQ_MAX_SIZE);
        ...
        (void)memcpy(pmpriv->adapter, pkey_material->key_param_set.key_params.aes.key, pkey->key_material,
                     pkey->key_len);
    ...
    {
        if (pkey->key_flags & (KEY_FLAG_RX_SEQ_VALID | KEY_FLAG_TX_SEQ_VALID))
            (void)memcpy(pmpriv->adapter, pkey_material->key_param_set.key_params.cmac_aes.ipn, pkey->pn, SEQ_MAX_SIZE);
        pkey_material->key_param_set.key_info &= ~KEY_INFO_MCAST_KEY;
        ...
        (void)memcpy(pmpriv->adapter, pkey_material->key_param_set.key_params.cmac_aes.key, pkey->key_material,
                     pkey->key_len);
    ...
    {
        if (pkey->key_flags & (KEY_FLAG_RX_SEQ_VALID | KEY_FLAG_TX_SEQ_VALID))
            (void)memcpy(pmpriv->adapter, pkey_material->key_param_set.key_params.tkip.pn, pkey->pn, SEQ_MAX_SIZE);
        ...
        (void)memcpy(pmpriv->adapter, pkey_material->key_param_set.key_params.tkip.key, pkey->key_material,
                     pkey->key_len);
    ...
    IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *cmd, IN t_u16 cmd_action, IN t_u32 cmd_oid, IN t_void *pdata_buf)
{
    ...
    if (cmd_action == HostCmd_ACT_GEN_GET)
    {
        cmd->size = wlan_cpu_to_le16(sizeof(pkey_material->action) + S_DS_GEN + KEYPARAMSET_FIXED_LEN +
                                     sizeof(MrvlIEtypesHeader_t));
        (void)memset(pmpriv->adapter, &pkey_material->key_param_set, 0, sizeof(MrvlIEtype_KeyParamSet_t));
        pkey_material->key_param_set.type   = wlan_cpu_to_le16(TLV_TYPE_KEY_MATERIAL);
        pkey_material->key_param_set.length = wlan_cpu_to_le16(KEYPARAMSET_FIXED_LEN);
        if (pkey->key_flags & KEY_FLAG_GROUP_KEY)
            pkey_material->key_param_set.key_info |= KEY_INFO_MCAST_KEY;
        else
            pkey_material->key_param_set.key_info |= KEY_INFO_UCAST_KEY;
    ...
    if (!pkey)
    {
        (void)memset(pmpriv->adapter, &pkey_material->key_param_set, 0,
                     (MRVL_NUM_WEP_KEY * sizeof(MrvlIEtype_KeyParamSet_t)));
        ret       = wlan_set_keyparamset_wep(pmpriv, &pkey_material->key_param_set, &key_param_len);
        cmd->size = wlan_cpu_to_le16(key_param_len + sizeof(pkey_material->action) + S_DS_GEN);
        goto done;
    }
    else
#endif /* KEY_MATERIAL_WEP */
    ...
    if (pkey->is_wapi_key)
    {
        ...
        pkey_material->key_param_set.key_type_id = wlan_cpu_to_le16(KEY_TYPE_ID_WAPI);
        if (cmd_oid == KEY_INFO_ENABLED)
            pkey_material->key_param_set.key_info = wlan_cpu_to_le16(KEY_INFO_WAPI_ENABLED);
        else
            pkey_material->key_param_set.key_info = !(wlan_cpu_to_le16(KEY_INFO_WAPI_ENABLED));
        ...
        { /* WAPI group key: multicast */
            pkey_material->key_param_set.key_info |= wlan_cpu_to_le16(KEY_INFO_WAPI_MCAST);
            pmpriv->sec_info.wapi_key_on = MTRUE;
        }

        pkey_material->key_param_set.type    = wlan_cpu_to_le16(TLV_TYPE_KEY_MATERIAL);
        pkey_material->key_param_set.key_len = wlan_cpu_to_le16(WAPI_KEY_LEN);
        (void)memcpy(pmpriv->adapter, &pkey_material->key_param_set.key[2], pkey->key_material, pkey->key_len);
        (void)memcpy(pmpriv->adapter, &pkey_material->key_param_set.key[2 + pkey->key_len], pkey->pn, PN_SIZE);
        pkey_material->key_param_set.length = wlan_cpu_to_le16(WAPI_KEY_LEN + KEYPARAMSET_FIXED_LEN);

        key_param_len = (WAPI_KEY_LEN + KEYPARAMSET_FIXED_LEN) + sizeof(MrvlIEtypesHeader_t);
        cmd->size     = wlan_cpu_to_le16(key_param_len + sizeof(pkey_material->action) + S_DS_GEN);
        goto done;
    }
#endif /* WAPI */
    ...
    if (pkey->key_len == WPA_AES_KEY_LEN)
    {
        ...
        pkey_material->key_param_set.key_type_id = wlan_cpu_to_le16(KEY_TYPE_ID_AES);
        if (cmd_oid == KEY_INFO_ENABLED)
...
#ifdef ENABLE_802_11W
    else if ((pkey->key_flags & KEY_FLAG_AES_MCAST_IGTK) && pkey->key_len == WPA_IGTK_KEY_LEN)
    {
        PRINTM(MCMND, "WPA_AES_CMAC\n");
        pkey_material->key_param_set.key_type_id = wlan_cpu_to_le16(KEY_TYPE_ID_AES_CMAC);
        if (cmd_oid == KEY_INFO_ENABLED)
    ...
    {
        pkey_material->key_param_set.type    = wlan_cpu_to_le16(TLV_TYPE_KEY_MATERIAL);
        pkey_material->key_param_set.key_len = wlan_cpu_to_le16((t_u16)pkey->key_len);
        (void)memcpy(pmpriv->adapter, pkey_material->key_param_set.key, pkey->key_material, pkey->key_len);
        pkey_material->key_param_set.length = wlan_cpu_to_le16((t_u16)pkey->key_len + KEYPARAMSET_FIXED_LEN);
        key_param_len = (t_u16)(pkey->key_len + KEYPARAMSET_FIXED_LEN) + sizeof(MrvlIEtypesHeader_t);

#ifdef ENABLE_802_11W
        /* key format with pn field is defined in Key Material V1 */
        if (pkey_material->key_param_set.key_type_id == wlan_cpu_to_le16(KEY_TYPE_ID_AES_CMAC))
        {
            cmac_param *param;
            param = (cmac_param *)pkey_material->key_param_set.key;
            (void)memcpy(pmpriv->adapter, param->ipn, pkey->pn, SEQ_MAX_SIZE);
            (void)memcpy(pmpriv->adapter, param->key, pkey->key_material, pkey->key_len);

            pkey_material->key_param_set.key_len = wlan_cpu_to_le16((t_u16)sizeof(cmac_param));
            pkey_material->key_param_set.length  = wlan_cpu_to_le16((t_u16)sizeof(cmac_param) + KEYPARAMSET_FIXED_LEN);
            key_param_len = (t_u16)(sizeof(cmac_param) + KEYPARAMSET_FIXED_LEN) + sizeof(MrvlIEtypesHeader_t);
        }
#endif

#ifdef ENABLE_IBSS_WPA2
        if ((pmpriv->bss_mode == MLAN_BSS_MODE_IBSS) && (pmpriv->sec_info.wpa2_enabled))
        {
            tlv              = (MrvlIEtypes_MacAddr_t *)((t_u8 *)&pkey_material->key_param_set + key_param_len);
            tlv->header.type = wlan_cpu_to_le16(TLV_TYPE_STA_MAC_ADDRESS);
            tlv->header.len  = wlan_cpu_to_le16(MLAN_MAC_ADDR_LENGTH);
            (void)memcpy(pmpriv->adapter, tlv->mac, pkey->mac_addr, MLAN_MAC_ADDR_LENGTH);
            cmd->size = wlan_cpu_to_le16(key_param_len + sizeof(pkey_material->action) + S_DS_GEN +
                                                  ...
                                                  IN t_void *pdata_buf)
{
    ...
    if (psk->ssid.ssid_len)
    {
        ...
        (void)memcpy(pmpriv->adapter, (char *)pssid_tlv->ssid, psk->ssid.ssid,
    ...
    if (memcmp(pmpriv->adapter, (t_u8 *)&psk->bssid, zero_mac, sizeof(zero_mac)))
    {
        ...
        (void)memcpy(pmpriv->adapter, pbssid_tlv->bssid, (t_u8 *)&psk->bssid, MLAN_MAC_ADDR_LENGTH);
    ...
    if (psk->psk_type == MLAN_PSK_PASSPHRASE)
    {
        ...
        (void)memcpy(pmpriv->adapter, ppassphrase_tlv->passphrase, psk->psk.passphrase.passphrase,
    ...
    if (psk->psk_type == MLAN_PSK_PASSWORD)
    {
        ...
        (void)memcpy(pmpriv->adapter, ppassword_tlv->password, psk->password.password,
    ...
    if (psk->psk_type == MLAN_PSK_PMK)
    {
        ...
        (void)memcpy(pmpriv->adapter, ppmk_tlv->pmk, psk->psk.pmk.pmk, MLAN_MAX_KEY_LENGTH);
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_cmd_802_11_supplicant_profile(IN pmlan_private pmpriv,
                                                      IN HostCmd_DS_COMMAND *cmd,
                                                      IN t_u16 cmd_action,
                                                      IN t_void *pdata_buf)
{
    HostCmd_DS_802_11_SUPPLICANT_PROFILE *sup_profile = &cmd->params.esupplicant_profile;
    ...
    cmd->size           = wlan_cpu_to_le16(sizeof(HostCmd_DS_802_11_SUPPLICANT_PROFILE) + S_DS_GEN - 1);
    ...
    if ((cmd_action == HostCmd_ACT_GEN_SET) && pdata_buf)
    {
        ...
        if (esupp->rsn_mode)
        {
            ...
            encr_proto_tlv->header.type = wlan_cpu_to_le16(TLV_TYPE_ENCRYPTION_PROTO);
            encr_proto_tlv->header.len  = (t_u16)sizeof(encr_proto_tlv->rsn_mode);
            encr_proto_tlv->rsn_mode    = wlan_cpu_to_le16(esupp->rsn_mode);
            ptlv_buffer += (encr_proto_tlv->header.len + sizeof(MrvlIEtypesHeader_t));
            cmd->size += (encr_proto_tlv->header.len + sizeof(MrvlIEtypesHeader_t));
            encr_proto_tlv->header.len = wlan_cpu_to_le16(encr_proto_tlv->header.len);
        }
        if (esupp->act_paircipher || esupp->act_groupcipher)
        {
            ...
            pcipher_tlv->header.len  = (t_u16)(sizeof(pcipher_tlv->pair_cipher) + sizeof(pcipher_tlv->group_cipher));
            if (esupp->act_paircipher)
            {
                pcipher_tlv->pair_cipher = wlan_cpu_to_le16(esupp->act_paircipher);
            }
            if (esupp->act_groupcipher)
            {
                pcipher_tlv->group_cipher = wlan_cpu_to_le16(esupp->act_groupcipher);
            }
            ptlv_buffer += (pcipher_tlv->header.len + sizeof(MrvlIEtypesHeader_t));
            cmd->size += (pcipher_tlv->header.len + sizeof(MrvlIEtypesHeader_t));
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_cmd_ibss_coalescing_status(IN pmlan_private pmpriv,
                                                   IN HostCmd_DS_COMMAND *cmd,
                                                   IN t_u16 cmd_action,
                                                   IN t_void *pdata_buf)
{
    ...
    cmd->size          = wlan_cpu_to_le16(sizeof(HostCmd_DS_802_11_IBSS_STATUS) + S_DS_GEN);
    cmd->result        = 0;
                                         ...
                                         IN t_void *pdata_buf)
{
    ...
    if (pmgmt_ie_list && cust_ie)
    {
        ...
        if (cust_ie->len)
            (void)memcpy(pmpriv->adapter, pmgmt_ie_list->ds_mgmt_ie.ie_data_list, cust_ie->ie_data_list, cust_ie->len);
    }
 ...
 *  @return 	   	MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_cmd_sysclock_cfg(IN pmlan_private pmpriv,
                                         IN HostCmd_DS_COMMAND *cmd,
                                         IN t_u16 cmd_action,
                                         IN t_void *pdata_buf)
{
    ...
    cmd->size    = wlan_cpu_to_le16(sizeof(HostCmd_DS_ECL_SYSTEM_CLOCK_CONFIG) + S_DS_GEN);
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_cmd_inactivity_timeout(IN HostCmd_DS_COMMAND *cmd, IN t_u16 cmd_action, IN t_void *pdata_buf)
{
    ...
    if (cmd_action == HostCmd_ACT_GEN_SET)
    {
        cmd_inac_to->timeout_unit     = wlan_cpu_to_le16((t_u16)inac_to->timeout_unit);
        cmd_inac_to->unicast_timeout  = wlan_cpu_to_le16((t_u16)inac_to->unicast_timeout);
        cmd_inac_to->mcast_timeout    = wlan_cpu_to_le16((t_u16)inac_to->mcast_timeout);
        cmd_inac_to->ps_entry_timeout = wlan_cpu_to_le16((t_u16)inac_to->ps_entry_timeout);
    }
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_cmd_low_pwr_mode(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *cmd, IN t_void *pdata_buf)
{
                                     ...
                                     IN t_void *pcmd_buf)
{
    ...
    switch (cmd_no)
    {
        ...
        case HostCmd_CMD_VERSION_EXT:
            (void)memset(pmpriv->adapter, cmd_ptr, 0x00, sizeof(HostCmd_DS_COMMAND));
        ...
        case HostCmd_CMD_RX_MGMT_IND:
            ...
            cmd_ptr->params.rx_mgmt_ind.mgmt_subtype_mask = wlan_cpu_to_le32((t_u32)(*((t_u32 *)pdata_buf)));
        ...
        case HostCmd_CMD_SUPPLICANT_PROFILE:
            ret = wlan_cmd_802_11_supplicant_profile(pmpriv, cmd_ptr, cmd_action, pdata_buf);
            break;
#endif /* CONFIG_MLAN_WMSDK */
...
mlan_status wlan_ops_sta_init_cmd(IN t_void *priv, IN t_u8 first_bss)
{
    ...
    /* get tx rate */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_TX_RATE_CFG, HostCmd_ACT_GEN_GET, 0, MNULL, MNULL);
    if (ret)
    {
        ret = MLAN_STATUS_FAILURE;
        goto done;
    ...
    /* get tx power */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_RF_TX_POWER, HostCmd_ACT_GEN_GET, 0, MNULL, MNULL);
    if (ret)
    {
        ret = MLAN_STATUS_FAILURE;
        goto done;
    ...
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_IBSS_COALESCING_STATUS, HostCmd_ACT_GEN_SET, 0, MNULL, &enable);
    if (ret)
    {
        ret = MLAN_STATUS_FAILURE;
        goto done;
    ...
    (void)memset(pmpriv->adapter, &amsdu_aggr_ctrl, 0, sizeof(amsdu_aggr_ctrl));
    amsdu_aggr_ctrl.enable = MLAN_ACT_ENABLE;
    /* Send request to firmware */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_AMSDU_AGGR_CTRL, HostCmd_ACT_GEN_SET, 0, MNULL,
    ...
    /* set MAC Control */
    ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_MAC_CONTROL, HostCmd_ACT_GEN_SET, 0, MNULL, &pmpriv->curr_pkt_filter);
    if (ret)
    {
        ret = MLAN_STATUS_FAILURE;
        goto done;
    ...
    {
        /* Get MAC address */
        ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_MAC_ADDRESS, HostCmd_ACT_GEN_GET, 0, MNULL, MNULL);
        if (ret)
        {
            ret = MLAN_STATUS_FAILURE;
            goto done;
