...
/* call it only in sdio lock */
#if CONFIG_FW_VDLL
static void wait_for_cmd_dnld_ready()
{
    /* here we already hold cmd lock, so only VDLL process will set cmd_sent */
    while (mlan_adap->cmd_sent)
    {
        wifi_sdio_unlock();
        OSA_TimeDelay(1);
        (void)wifi_sdio_lock();
...
#else
static inline void wait_for_cmd_dnld_ready()
{
}
#endif
...
static mlan_status wlan_handle_cmd_resp_packet(t_u8 *pmbuf)
{
    ...
    switch (cmdtype)
    {
        ...
        case HostCmd_CMD_802_11_MAC_ADDRESS:
            ...
            {
                wifi_get_mac_address_from_cmdresp((HostCmd_DS_COMMAND *)(void *)cmdresp, dev_mac_addr_wfd);
            }
            else
            {
...
static void wlan_reconfigure_tx_buffers(void)
{
    ...
    /* sdiopkt = outbuf */
    wifi_prepare_reconfigure_tx_buf_cmd(&sdiopkt->hostcmd, wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[BSS_TYPE_STA]));
    sdiopkt->pkttype = MLAN_TYPE_CMD;
...
static void wlan_get_channel_region_cfg(void)
{
    ...
    /* sdiopkt = outbuf */
    wifi_prepare_get_channel_region_cfg_cmd(&sdiopkt->hostcmd, wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[BSS_TYPE_STA]));
...
static void wlan_get_hw_spec(void)
{
    ...
    (void)memset(outbuf, 0, buflen);
    /* sdiopkt = outbuf */
    wifi_prepare_get_hw_spec_cmd(&sdiopkt->hostcmd, wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[BSS_TYPE_STA]));
...
static void wlan_get_mac_addr_sta(void)
{
    ...
    /* sdiopkt = outbuf */
    wifi_prepare_get_mac_addr_cmd(&sdiopkt->hostcmd, wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[BSS_TYPE_STA]));
...
static void wlan_get_fw_ver_ext(int version_str_sel)
{
    ...
    /* sdiopkt = outbuf */
    wifi_prepare_get_fw_ver_ext_cmd(&sdiopkt->hostcmd, wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[BSS_TYPE_STA]), version_str_sel);
...
static void wlan_get_value_1(void)
{
    ...
    /* sdiopkt = outbuf */
    wifi_prepare_get_value1(&sdiopkt->hostcmd, wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[BSS_TYPE_STA]));
...
static void _wlan_set_mac_addr(void)
{
    ...
    /* sdiopkt = outbuf */
    wifi_prepare_set_mac_addr_cmd(&sdiopkt->hostcmd, wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[BSS_TYPE_STA]));
...
static void wlan_set_11n_cfg(void)
{
    ...
    /* sdiopkt = outbuf */
    sdiopkt->hostcmd.seq_num = wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[BSS_TYPE_STA]);
    sdiopkt->pkttype         = MLAN_TYPE_CMD;
    sdiopkt->size    = sdiopkt->hostcmd.size + INTF_HEADER_LEN;
...
static void wlan_enable_amsdu(void)
{
    ...
    /* sdiopkt = outbuf */
    wifi_prepare_enable_amsdu_cmd(&sdiopkt->hostcmd, wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[BSS_TYPE_STA]));
...
static void wlan_set_mac_ctrl(void)
{
    ...
    /* sdiopkt = outbuf */
    wlan_prepare_mac_control_cmd(&sdiopkt->hostcmd, wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[BSS_TYPE_STA]));
...
static int wlan_set_low_power_mode()
{
    ...
    wifi_prepare_low_power_mode_cmd(&sdiopkt->hostcmd, wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[BSS_TYPE_STA]));
...
#if CONFIG_WIFI_IO_DUMP
    (void)PRINTF("OUT_CMD");
    dump_hex(buf, sdio->size);
#endif /* CONFIG_WIFI_IO_DUMP */
#if defined(SD8801)
    sdio_drv_write(mlan_adap->ioport, 1, tx_blocks, buflen, (t_u8 *)buf, &resp);
#elif defined(SD8978) || defined(SD8987) || defined(SD8997) || defined(SD9097) || defined(SD9098) || defined(SD9177) || defined(IW610)
    (void)sdio_drv_write(mlan_adap->ioport | CMD_PORT_SLCT, 1, tx_blocks, buflen, (t_u8 *)buf, &resp);
#endif
...
#endif
#if CONFIG_FW_VDLL
    mlan_adap->vdll_ctrl.cmd_buf = (void *)wifi_get_vdllcommand_buffer();
#endif
    txportno = 0;
    return mlanstatus;
}
...
static bool sdio_fw_is_hang(void)
{
#if 0
