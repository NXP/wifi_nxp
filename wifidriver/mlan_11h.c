...
static const IEEEtypes_SupportChan_Subband_t wlan_11h_2_4G_region_JPN40 = {14, 1};
...
static const IEEEtypes_SupportChan_Subband_t wlan_11h_unii_middle_band = {52, 4};

/** U-NII sub-band config : Start Channel = 100, NumChans = 11 */
static const IEEEtypes_SupportChan_Subband_t wlan_11h_unii_mid_upper_band = {100, 11};

/** U-NII sub-band config : Start Channel = 149, NumChans = 5 */
static const IEEEtypes_SupportChan_Subband_t wlan_11h_unii_upper_band = {149, 5};
#endif
 ...
 */
static t_u32 wlan_11h_convert_ieee_to_mrvl_ie(mlan_adapter *pmadapter, t_u8 *pout_buf, const t_u8 *pin_ie)
{
 ...
 */
static t_u32 wlan_11h_set_ibss_dfs_ie(mlan_private *priv, IEEEtypes_IBSS_DFS_t *pdfs)
{
    t_u8 num_chans = 0;
    ...
    for (; (num_chans < adapter->parsed_region_chan.no_of_chan) && (num_chans < WLAN_11H_MAX_IBSS_DFS_CHANNELS);
         num_chans++)
    {
        pdfs->channel_map[num_chans].channel_number = adapter->parsed_region_chan.chan_pwr[num_chans].chan;
    ...
    if (num_chans)
    {
        ...
        pdfs->len        = (sizeof(pdfs->dfs_owner) + sizeof(pdfs->dfs_recovery_interval) +
 ...
 *    - 0 if returned element is not setup
 */
static t_u16 wlan_11h_set_supp_channels_ie(mlan_private *priv, t_u8 band, IEEEtypes_SupportedChannels_t *psup_chan)
{
    ...
    if (num_subbands)
    {
        ...
        ret_len = (t_u16)(psup_chan->len + sizeof(psup_chan->len) + sizeof(psup_chan->element_id));
 ...
 */
static mlan_status wlan_11h_cmd_tpc_request(mlan_private *priv, HostCmd_DS_COMMAND *pcmd_ptr, const t_void *pinfo_buf)
{
    ENTER();
    ...
    HEXDUMP("11h: 11_TPC_ADAPT_REQ:", (t_u8 *)pcmd_ptr, (t_u32)pcmd_ptr->size);
 ...
 *  @return          MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_11h_cmd_tpc_info(mlan_private *priv, HostCmd_DS_COMMAND *pcmd_ptr, const t_void *pinfo_buf)
{
    HostCmd_DS_802_11_TPC_INFO *ptpc_info           = &pcmd_ptr->params.tpc_info;
    MrvlIEtypes_LocalPowerConstraint_t *pconstraint = &ptpc_info->local_constraint;
    MrvlIEtypes_PowerCapability_t *pcap             = &ptpc_info->power_cap;

    wlan_11h_device_state_t *pstate                  = &priv->adapter->state_11h;
    const wlan_11h_tpc_info_param_t *ptpc_info_param = (wlan_11h_tpc_info_param_t *)pinfo_buf;
 ...
 *  @return          MLAN_STATUS_SUCCESS
 */
static mlan_status wlan_11h_cmd_chan_sw_ann(mlan_private *priv, HostCmd_DS_COMMAND *pcmd_ptr, const t_void *pinfo_buf)
{
    const HostCmd_DS_802_11_CHAN_SW_ANN *pch_sw_ann = (HostCmd_DS_802_11_CHAN_SW_ANN *)pinfo_buf;
 ...
 */
static mlan_status wlan_11h_cmd_chan_rpt_req(mlan_private *priv, HostCmd_DS_COMMAND *pcmd_ptr, const t_void *pinfo_buf)
{
    const HostCmd_DS_CHAN_RPT_REQ *pchan_rpt_req = (HostCmd_DS_CHAN_RPT_REQ *)pinfo_buf;
...
static t_u32 wlan_11h_set_local_power_constraint_tlv(
    t_u8 **ppbuffer, t_u8 channel, t_u8 power_constraint, t_u8 min_tx_power_capability, t_u8 max_tx_power_capability)
{
    ...
    PRINTM(MINFO, "11h: Set local power constraint = %d channel=%d min_tx_pwr=%d max_tx_pwr=%d\n", power_constraint,
           channel, min_tx_power_capability, max_tx_power_capability);
 ...
 *                       buffer (ppbuffer)
 */
static t_u32 wlan_11h_process_infra_join(
    mlan_private *priv, t_u8 **ppbuffer, t_u8 band, t_u32 channel, wlan_11h_bss_info_t *p11h_bss_info)
{
    ...
    ret_len += wlan_11h_set_local_power_constraint_tlv(
        ppbuffer, (t_u8)channel, (t_u8)p11h_bss_info->power_constraint.local_constraint,
        (t_u8)priv->adapter->state_11h.min_tx_power_capability, (t_u8)priv->adapter->state_11h.max_tx_power_capability);
 ...
 *                       buffer (ppbuffer)
 */
static t_u32 wlan_11h_process_adhoc(mlan_private *priv,
                                    t_u8 **ppbuffer,
                                    t_u32 channel,
                                    wlan_11h_bss_info_t *p11h_bss_info)
{
    ...
    size_appended = wlan_11h_convert_ieee_to_mrvl_ie(adapter, (t_u8 *)*ppbuffer, (t_u8 *)&dfs_elem);
    ...
    *ppbuffer += size_appended;
    ...
    if (p11h_bss_info)
    {
        ...
        {
            size_appended = wlan_11h_convert_ieee_to_mrvl_ie(adapter, (t_u8 *)*ppbuffer, (t_u8 *)&p11h_bss_info->quiet);
    ...
    else
    {
        ...
        {
            size_appended =
                wlan_11h_convert_ieee_to_mrvl_ie(adapter, (t_u8 *)*ppbuffer, (t_u8 *)&adapter->state_11h.quiet_ie);
            HEXDUMP("11h: Quiet", (t_u8 *)*ppbuffer, size_appended);
            *ppbuffer += size_appended;
            ret_len += size_appended;
        }
    ...
    ret_len += wlan_11h_set_local_power_constraint_tlv(ppbuffer, (t_u8)channel, (t_u8)local_constraint,
...
static t_bool wlan_11h_is_slave_active_on_dfs_chan(mlan_private *priv)
{
    ...
    ENTER();
    if ((priv->media_connected == MTRUE) && (priv->curr_bss_params.band & BAND_A) &&
        wlan_11h_radar_detect_required(priv, priv->curr_bss_params.bss_descriptor.channel))
        ret = MTRUE;
...
static t_bool wlan_11h_is_master_active_on_dfs_chan(mlan_private *priv)
{
    ...
    {
        /* Ad-hoc creator */
        if (((priv->media_connected == MTRUE) || (priv->adhoc_state == ADHOC_STARTING)) &&
            (priv->adapter->adhoc_start_band & BAND_A) && wlan_11h_radar_detect_required(priv, priv->adhoc_channel))
            ret = MTRUE;
    }
    else if (GET_BSS_ROLE(priv) == MLAN_BSS_ROLE_UAP)
    {
        /* UAP */
#ifdef UAP_SUPPORT
        if ((priv->uap_bss_started == MTRUE) && (priv->uap_state_chan_cb.band_config & BAND_CONFIG_5GHZ) &&
            wlan_11h_radar_detect_required(priv, priv->uap_state_chan_cb.channel))
            ret = MTRUE;
#endif
    }
    LEAVE();
...
static t_bool wlan_11h_is_dfs_master(mlan_private *priv)
{
    ...
    else if ((GET_BSS_ROLE(priv) == MLAN_BSS_ROLE_STA) && (priv->bss_mode == MLAN_BSS_MODE_IBSS) &&
             (priv->adhoc_state == ADHOC_STARTED || priv->adhoc_state == ADHOC_STARTING))
    {
        ret = MTRUE;
    }
 ...
 *  @return             MTRUE or MFALSE
 */
static t_bool wlan_is_intf_active(mlan_private *pmpriv)
{
    t_bool ret = MFALSE;
    ENTER();
 ...
 */
static t_bool wlan_11h_check_radar_det_state(mlan_adapter *pmadapter, OUT t_u32 *pnew_state)
{
    ...
    if (pmadapter->state_11h.master_radar_det_enable_pending ||
        (wlan_count_priv_cond(pmadapter, wlan_11h_is_master_active_on_dfs_chan, wlan_11h_is_dfs_master) > 0))
        radar_det_state_new |= MASTER_RADAR_DET_MASK;
    if (pmadapter->state_11h.slave_radar_det_enable_pending ||
        (wlan_count_priv_cond(pmadapter, wlan_11h_is_slave_active_on_dfs_chan, wlan_11h_is_dfs_slave) > 0))
    ...
    ret = (wlan_11h_get_current_radar_detect_flags(pmadapter) != radar_det_state_new) ? MTRUE : MFALSE;
    if (ret)
 ...
 *  @return MTRUE or MFALSE
 */
static t_bool wlan_only_uap_priv_in_list(mlan_private **priv_list, t_u8 priv_list_count)
{
 ...
 */
static mlan_status wlan_11h_prepare_custom_ie_chansw(IN mlan_adapter *pmadapter,
                                                     ...
                                                     IN t_bool is_adding_ie)
{
    ...
    ret = pmadapter->callbacks.moal_malloc(pmadapter->pmoal_handle, sizeof(mlan_ioctl_req) + sizeof(mlan_ds_misc_cfg),
                                           MLAN_MEM_DEF, (t_u8 **)&pioctl_req);
    ...
    pchansw_ie->len               = sizeof(IEEEtypes_ChanSwitchAnn_t) - sizeof(IEEEtypes_Header_t);
    ...
    DBG_HEXDUMP(MCMD_D, "11h: custom_ie containing CHAN_SW IE", (t_u8 *)pcust_chansw_ie,
                pds_misc_cfg->param.cust_ie.len);
 ...
 */
static t_u8 wlan_11h_get_uap_start_channel(mlan_private *priv, t_u8 uap_band_cfg)
{
    t_u8 start_chn;
    mlan_adapter *adapter = priv->adapter;
    t_u32 region;
    ...
    if (uap_band_cfg & UAP_BAND_CONFIG_5GHZ)
    {
        ...
        if (wlan_11h_is_enabled(priv))
        {
            ...
            for (region = 0; (region < MAX_REGION_CHANNEL_NUM); region++)
            {
                ...
                if (chn_tbl->valid && chn_tbl->region == adapter->region_code && chn_tbl->band & BAND_A)
                {
                    /*
                    ...
                    do
                    {
                        rand_entry = wlan_11h_get_random_num(adapter) % chn_tbl->num_cfp;
 ...
 */
static wlan_dfs_timestamp_t *wlan_11h_find_dfs_timestamp(mlan_adapter *pmadapter, t_u8 channel)
{
    wlan_dfs_timestamp_t *pts = MNULL, *pts_found = MNULL;

    ENTER();
    pts = (wlan_dfs_timestamp_t *)util_peek_list(pmadapter->pmoal_handle, &pmadapter->state_dfs.dfs_ts_head, MNULL,
                                                 MNULL);

    while (pts && pts != (wlan_dfs_timestamp_t *)&pmadapter->state_dfs.dfs_ts_head)
    {
        PRINTM(MINFO,
               "dfs_timestamp(@ %p) - chan=%d, repr=%d(%s),"
               " time(sec.usec)=%lu.%06lu\n",
               pts, pts->channel, pts->represents, DFS_TS_REPR_STRINGS[pts->represents], pts->ts_sec, pts->ts_usec);
 ...
 */
static t_void wlan_11h_remove_dfs_timestamp(mlan_adapter *pmadapter, wlan_dfs_timestamp_t *pdfs_ts)
{
    ENTER();
    // dequeue and delete timestamp
    util_unlink_list(pmadapter->pmoal_handle, &pmadapter->state_dfs.dfs_ts_head, (pmlan_linked_list)pdfs_ts, MNULL,
                     MNULL);
 ...
 *  @return           Pointer to timestamp if found, or MNULL
 */
static mlan_status wlan_11h_add_dfs_timestamp(mlan_adapter *pmadapter, t_u8 repr, t_u8 channel)
{
    ...
    if (!pdfs_ts)
    {
        ...
        ret = pmadapter->callbacks.moal_malloc(pmadapter->pmoal_handle, sizeof(wlan_dfs_timestamp_t), MLAN_MEM_DEF,
                                               (t_u8 **)&pdfs_ts);
    ...
    // update params
    pmadapter->callbacks.moal_get_system_time(pmadapter->pmoal_handle, &pdfs_ts->ts_sec, &pdfs_ts->ts_usec);
    pdfs_ts->represents = repr;

    PRINTM(MCMD_D,
           "11h: add/update dfs_timestamp - chan=%d, repr=%d(%s),"
           " time(sec.usec)=%lu.%06lu\n",
           pdfs_ts->channel, pdfs_ts->represents, DFS_TS_REPR_STRINGS[pdfs_ts->represents], pdfs_ts->ts_sec,
           pdfs_ts->ts_usec);
...
mlan_status wlan_11h_config_master_radar_det(mlan_private *priv, t_bool enable)
{
    ...
    ENTER();
    if (wlan_11h_is_dfs_master(priv) && priv->adapter->init_para.dfs_master_radar_det_en)
    {
...
mlan_status wlan_11h_config_slave_radar_det(mlan_private *priv, t_bool enable)
{
    ...
    ENTER();
    if (wlan_11h_is_dfs_slave(priv) && priv->adapter->init_para.dfs_slave_radar_det_en)
    {
...
mlan_status wlan_11h_check_update_radar_det_state(mlan_private *pmpriv)
{
    ...
    if (wlan_11h_check_radar_det_state(pmpriv->adapter, &new_radar_det_state))
    {
        ...
        ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_SNMP_MIB, HostCmd_ACT_GEN_SET, Dot11H_i, MNULL, &mib_11h);
        if (ret)
 ...
 *  @return      MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_11h_activate(mlan_private *priv, t_void *pioctl_buf, t_bool flag)
{
     ...
     * Send cmd to FW to enable/disable 11h function in firmware
     */
    ret = wlan_prepare_cmd(priv, HostCmd_CMD_802_11_SNMP_MIB, HostCmd_ACT_GEN_SET, Dot11H_i, (t_void *)pioctl_buf,
                           &enable);
    if (ret)
        ret = MLAN_STATUS_FAILURE;
    else
...
t_void wlan_11h_cleanup(mlan_adapter *adapter)
{
    ...
    pdfs_ts = (wlan_dfs_timestamp_t *)util_peek_list(adapter->pmoal_handle, &pstate_dfs->dfs_ts_head, MNULL, MNULL);
    while (pdfs_ts)
    {
        util_unlink_list(adapter->pmoal_handle, &pstate_dfs->dfs_ts_head, (pmlan_linked_list)pdfs_ts, MNULL, MNULL);
...
t_u8 wlan_11h_get_adhoc_start_channel(mlan_private *priv)
{
     ...
     * Check that we are looking for a channel in the A Band
     */
    if ((adapter->adhoc_start_band & BAND_A) || (adapter->adhoc_start_band & BAND_AN))
    {
        /*
         * Set default to the A Band default. Used if random selection fails
         *   or if 11h is not enabled
                ...
                if (chn_tbl->valid && chn_tbl->region == adapter->region_code && chn_tbl->band & BAND_A)
                {
                    ...
                    do
                    {
                        ...
                        start_chn  = (t_u8)chn_tbl->pcfp[rand_entry].channel;
                              ...
                              ((adapter->state_rdh.stage == RDH_GET_INFO_CHANNEL) &&
 ...
 */
t_s32 wlan_11h_issue_radar_detect(mlan_private *priv, pmlan_ioctl_req pioctl_req, t_u8 channel)
{
    t_s32 ret;
    ...
    if (ret)
    {
        ...
        chan_rpt_req.millisec_dwell_time = WLAN_11H_CHANNEL_AVAIL_CHECK_DURATION;
#ifdef DFS_TESTING_SUPPORT
        if (priv->adapter->dfs_test_params.user_cac_period_msec)
        {
            ...
            chan_rpt_req.millisec_dwell_time = priv->adapter->dfs_test_params.user_cac_period_msec;
        }
#endif
        ...
        ret = wlan_prepare_cmd(priv, HostCmd_CMD_CHAN_REPORT_REQUEST, HostCmd_ACT_GEN_SET, 0, (t_void *)pioctl_req,
                               (t_void *)&chan_rpt_req);
    }
...
mlan_status wlan_11h_check_chan_report(mlan_private *priv, t_u8 chan)
{
    ...
    priv->adapter->callbacks.moal_get_system_time(priv->adapter->pmoal_handle, &sec, &usec);
    ...
    if ((!pstate_dfs->dfs_check_pending) && (chan == pstate_dfs->dfs_check_channel) &&
        ((sec - pstate_dfs->dfs_report_time_sec) < MAX_DFS_REPORT_USABLE_AGE_SEC))
    {
 ...
 */
t_s32 wlan_11h_process_start(mlan_private *priv,
                             t_u8 **ppbuffer,
                             ...
                             wlan_11h_bss_info_t *p11h_bss_info)
{
    ...
    ENTER();
    if (wlan_11h_is_enabled(priv) && ((adapter->adhoc_start_band & BAND_A) || (adapter->adhoc_start_band & BAND_AN)))
    {
 ...
 */
t_s32 wlan_11h_process_join(mlan_private *priv,
                            t_u8 **ppbuffer,
                            IEEEtypes_CapInfo_t *pcap_info,
                            t_u8 band,
                            t_u32 channel,
                            wlan_11h_bss_info_t *p11h_bss_info)
{
    ...
    if (p11h_bss_info->sensed_11h)
    {
        ...
        if (pcap_info->ibss)
        {
            ...
            ret = wlan_11h_process_adhoc(priv, ppbuffer, channel, p11h_bss_info);
        }
        else
        {
            PRINTM(MINFO, "11h: Infra join: Sensed\n");
            ret = wlan_11h_process_infra_join(priv, ppbuffer, band, channel, p11h_bss_info);
        }
    }
    else
    {
 ...
 */
mlan_status wlan_11h_cmd_process(mlan_private *priv, HostCmd_DS_COMMAND *pcmd_ptr, const t_void *pinfo_buf)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 *  @return      MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_11h_cmdresp_process(mlan_private *priv, const HostCmd_DS_COMMAND *resp)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 *  @return              MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_11h_process_bss_elem(mlan_adapter *pmadapter, wlan_11h_bss_info_t *p11h_bss_info, const t_u8 *pelement)
{
    mlan_status ret  = MLAN_STATUS_SUCCESS;
...
mlan_status wlan_11h_handle_event_chanswann(mlan_private *priv)
{
    ...
    ret = wlan_disconnect(priv, MNULL, &priv->curr_bss_params.bss_descriptor.mac_address);
 ...
 *  @return MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_11h_ioctl_dfs_testing(pmlan_adapter pmadapter, pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_GET)
    {
        ...
        dfs_test->usr_no_chan_change  = pdfs_test_params->no_channel_change_on_radar;
        dfs_test->usr_fixed_new_chan  = pdfs_test_params->fixed_new_channel_on_radar;
    }
    else
    {
        ...
        pdfs_test_params->no_channel_change_on_radar = dfs_test->usr_no_chan_change;
        pdfs_test_params->fixed_new_channel_on_radar = dfs_test->usr_fixed_new_chan;
    }
...
t_bool wlan_11h_is_channel_under_nop(mlan_adapter *pmadapter, t_u8 channel)
{
    ...
    if (pdfs_ts && (pdfs_ts->channel == channel) && (pdfs_ts->represents == DFS_TS_REPR_NOP_START))
    {
        /* found NOP_start timestamp entry on channel */
        pmadapter->callbacks.moal_get_system_time(pmadapter->pmoal_handle, &now_sec, &now_usec);
#ifdef DFS_TESTING_SUPPORT
        if (pmadapter->dfs_test_params.user_nop_period_sec)
        {
            ...
            if ((now_sec - pdfs_ts->ts_sec) <= pmadapter->dfs_test_params.user_nop_period_sec)
            {
                ret = MTRUE;
            }
        }
 ...
 *  @return MLAN_STATUS_SUCCESS or MLAN_STATUS_FAILURE
 */
mlan_status wlan_11h_handle_event_chanrpt_ready(mlan_private *priv, mlan_event *pevent)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
    ...
    DBG_HEXDUMP(MCMD_D, "11h: Event ChanRptReady (HostCmd_DS_CHAN_RPT_RSP)", (t_u8 *)pChanRptRsp,
    ...
    if (wlan_le32_to_cpu(pChanRptRsp->cmd_result) == MLAN_CMD_RESULT_SUCCESS)
    {
        ...
        evtLen -= sizeof(HostCmd_DS_CHAN_RPT_RSP) - sizeof(pChanRptRsp->tlv_buffer);
            ...
            switch (wlan_le16_to_cpu(pTlv->header.type))
            {
                case TLV_TYPE_CHANRPT_11H_BASIC:
                    ...
                    if (pMeasRptBasic->radar)
                    {
                        ...
                        wlan_11h_add_dfs_timestamp(priv->adapter, DFS_TS_REPR_NOP_START, pstate_dfs->dfs_check_channel);
                    }
                    break;
    ...
    switch (pstate_rdh->stage)
    {
        case RDH_CHK_INTFS:
            ...
            if (pstate_rdh->priv_list_count == 0)
            {
                ...
                PRINTM(MMSG,
                       "11h: Radar Detected - no active priv's,"
                       " skip event handling.\n");
                pstate_rdh->stage = RDH_OFF;
        ...
        case RDH_GET_INFO_CHANNEL:
            ...
            {
                if (wlan_only_uap_priv_in_list(pstate_rdh->priv_list, pstate_rdh->priv_list_count))
                {
#ifdef UAP_SUPPORT
                    ...
                    pmpriv->uap_state_chan_cb.get_chan_callback = wlan_11h_radar_detected_callback;
                ...
                else
                {
                    ...
                    pstate_rdh->curr_channel = pmpriv->curr_bss_params.bss_descriptor.channel;
                }
            }
#ifdef UAP_SUPPORT
            ...
            /* add channel to NOP list */
            wlan_11h_add_dfs_timestamp(pmadapter, DFS_TS_REPR_NOP_START, pstate_rdh->curr_channel);
            ...
            do
            {
                ...
                if (GET_BSS_ROLE(pmpriv) == MLAN_BSS_ROLE_UAP)
                    pstate_rdh->new_channel =
                        wlan_11h_get_uap_start_channel(pmpriv, pmpriv->uap_state_chan_cb.band_config);
                else
#endif
                    pstate_rdh->new_channel = wlan_11h_get_adhoc_start_channel(pmpriv);
            } while ((pstate_rdh->new_channel == pstate_rdh->curr_channel) &&
                     (++i < MAX_RANDOM_CHANNEL_RETRIES)); /* avoid
                                                             deadloop
                                                           */
            ...
            if (pmadapter->dfs_test_params.fixed_new_channel_on_radar)
            {
                ...
                pstate_rdh->new_channel = pmadapter->dfs_test_params.fixed_new_channel_on_radar;
            }
#endif
        ...
        case RDH_GET_INFO_BEACON_DTIM:
            ...
            if (pstate_rdh->priv_curr_idx < pstate_rdh->priv_list_count)
            {
                ...
                PRINTM(MCMD_D, "%s():  uap.bcn_pd=%d, uap.dtim_pd=%d\n", __FUNCTION__,
                       pmpriv->uap_state_chan_cb.beacon_period, pmpriv->uap_state_chan_cb.dtim_period);
                bcn_dtim_msec = (pmpriv->uap_state_chan_cb.beacon_period * pmpriv->uap_state_chan_cb.dtim_period);
            ...
            while ((++pstate_rdh->priv_curr_idx) < pstate_rdh->priv_list_count)
            {
                ...
                { /* get STA info from driver and compare here */
                    ...
                    {
                        bcn_pd_msec = pmpriv->curr_bss_params.bss_descriptor.beacon_period;
        ...
        case RDH_SET_CUSTOM_IE:
            ...
            if (pstate_rdh->priv_curr_idx == RDH_STAGE_FIRST_ENTRY_PRIV_IDX)
            {
                ...
                ret = wlan_11h_prepare_custom_ie_chansw(pmadapter, &pioctl_req, MTRUE);
                if ((ret != MLAN_STATUS_SUCCESS) || !pioctl_req)
                {
                ...
                PRINTM(MMSG, "11h: Radar Detected - adding CHAN_SW IE to interfaces.\n");
                pmpriv                    = pstate_rdh->priv_list[0];
                pstate_rdh->priv_curr_idx = 0;
                ...
                {
                    PRINTM(MERROR, "%s(): Could not set IE for priv=%p [priv_bss_idx=%d]!\n", __FUNCTION__, pmpriv,
                           pmpriv->bss_index);
                    // TODO: how to handle this error case?? ignore & continue?
                }
                /* free ioctl buffer memory before we leave */
                pmadapter->callbacks.moal_mfree(pmadapter->pmoal_handle, (t_u8 *)pioctl_req);
                break; // EXIT CASE
            }
            // else
            pstate_rdh->priv_curr_idx = RDH_STAGE_FIRST_ENTRY_PRIV_IDX;
        ...
        case RDH_REM_CUSTOM_IE:
            ...
            if (pstate_rdh->priv_curr_idx == RDH_STAGE_FIRST_ENTRY_PRIV_IDX)
            {
                ...
                t_u16 delay_ms = MAX(MIN_RDH_CHAN_SW_IE_PERIOD_MSEC,
                                     MIN((5 * pstate_rdh->max_bcn_dtim_ms), MAX_RDH_CHAN_SW_IE_PERIOD_MSEC));
                PRINTM(MMSG,
                ...
                ret = wlan_11h_prepare_custom_ie_chansw(pmadapter, &pioctl_req, MFALSE);
                if ((ret != MLAN_STATUS_SUCCESS) || !pioctl_req)
                {
                    PRINTM(MERROR, "%s(): Error in preparng CHAN_SW IE.\n", __FUNCTION__);
                    break; // EXIT CASE
        ...
        case RDH_STOP_INTFS:
            ...
            while ((++pstate_rdh->priv_curr_idx) < pstate_rdh->priv_list_count)
            {
                ...
                if (GET_BSS_ROLE(pmpriv) == MLAN_BSS_ROLE_UAP)
                {
                    ret = wlan_prepare_cmd(pmpriv, HOST_CMD_APCMD_BSS_STOP, HostCmd_ACT_GEN_SET, 0, MNULL, MNULL);
                    break; // leads to exit case
                }
#endif
#ifdef STA_SUPPORT
        ...
        case RDH_RESTART_INTFS:
            ...
            while ((++pstate_rdh->priv_curr_idx) < pstate_rdh->priv_list_count)
            {
                ...
                if (GET_BSS_ROLE(pmpriv) == MLAN_BSS_ROLE_UAP)
                {
                    if (wlan_11h_radar_detect_required(pmpriv, pstate_rdh->new_channel))
                    {
                    ...
                    ret = wlan_prepare_cmd(pmpriv, HOST_CMD_APCMD_BSS_START, HostCmd_ACT_GEN_SET, 0, MNULL, MNULL);
                    break; // leads to exit case
                }
#endif
#ifdef STA_SUPPORT
                if (GET_BSS_ROLE(pmpriv) == MLAN_BSS_ROLE_STA)
                {
                    ...
                    if (wlan_11h_is_dfs_master(pmpriv))
                    {
                        ...
                        ret = wlan_prepare_cmd(pmpriv, HostCmd_CMD_802_11_AD_HOC_START, HostCmd_ACT_GEN_SET, 0, MNULL,
...
mlan_status wlan_11h_dfs_event_preprocessing(mlan_adapter *pmadapter)
{
    ...
    switch (pmadapter->event_cause & EVENT_ID_MASK)
    {
        case EVENT_RADAR_DETECTED:
            ...
            if (wlan_get_privs_by_two_cond(pmadapter, wlan_11h_is_master_active_on_dfs_chan, wlan_11h_is_dfs_master,
                                           MTRUE, priv_list))
            {
                pmpriv = priv_list[0];
                PRINTM(MINFO, "%s: found dfs_master priv=%p\n", __FUNCTION__, pmpriv);
            }
            else if (wlan_get_privs_by_two_cond(pmadapter, wlan_11h_is_slave_active_on_dfs_chan, wlan_11h_is_dfs_slave,
                                                MTRUE, priv_list))
            {
                pmpriv = priv_list[0];
...
mlan_status wlan_11h_switch_non_dfs_chan(mlan_private *priv, t_u8 *chan)
{
    ...
    for (i = 0; i < MAX_REGION_CHANNEL_NUM; i++)
    {
        if (pmadapter->region_channel[i].band == BAND_A && pmadapter->region_channel[i].valid)
        {
            chn_tbl = &pmadapter->region_channel[i];
            break;
        }
    ...
    if (rand_tries == MAX_SWITCH_CHANNEL_RETRIES)
    {
        for (i = 0; i < chn_tbl->num_cfp; i++)
        {
            if (chn_tbl->pcfp[i].passive_scan_or_radar_detect == MFALSE &&
                !wlan_11h_is_channel_under_nop(pmadapter, (t_u8)chn_tbl->pcfp[i].channel))
            {
