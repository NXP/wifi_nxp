...
mlan_status mlan_register(IN pmlan_device pmdevice, OUT t_void **ppmlan_adapter)
{
    ...
    if (pmdevice->max_tx_buf == MLAN_TX_DATA_BUF_SIZE_2K || pmdevice->max_tx_buf == MLAN_TX_DATA_BUF_SIZE_4K ||
 ...
 *
 */
mlan_status mlan_set_init_param(IN t_void *pmlan_adapter, IN pmlan_init_param pparam)
{
    mlan_status ret         = MLAN_STATUS_SUCCESS;
    mlan_adapter *pmadapter = (mlan_adapter *)pmlan_adapter;
...
mlan_status mlan_shutdown_fw(IN t_void *pmlan_adapter)
{
    ...
    if (pcb->moal_spin_lock(pmadapter->pmoal_handle, pmadapter->pmlan_lock) != MLAN_STATUS_SUCCESS)
    {
        ret = MLAN_STATUS_FAILURE;
        goto exit_shutdown_fw;
    }

    if (pcb->moal_spin_unlock(pmadapter->pmoal_handle, pmadapter->pmlan_lock) != MLAN_STATUS_SUCCESS)
    {
        ret = MLAN_STATUS_FAILURE;
        goto exit_shutdown_fw;
    }
...
process_start:
    do
    {
        ...
        if ((pmadapter->hw_status == WlanHardwareStatusClosing) || (pmadapter->hw_status == WlanHardwareStatusNotReady))
            break;
        ...
        if ((pmadapter->ps_state == PS_STATE_SLEEP) &&
            ...
            (util_peek_list(pmadapter->pmoal_handle, &pmadapter->cmd_pending_q, pcb->moal_spin_lock,
             ...
             !wlan_bypass_tx_list_empty(pmadapter) || !wlan_wmm_lists_empty(pmadapter)))
        {
        ...
        if (IS_CARD_RX_RCVD(pmadapter))
        {
            ...
            if (pmadapter->hs_activated == MTRUE)
            {
                ...
                wlan_host_sleep_activated_event(wlan_get_priv(pmadapter, MLAN_BSS_ROLE_ANY), MFALSE);
            }
        ...
        else
        {
            ...
            if (pmadapter->pm_wakeup_fw_try)
                break;
            if (pmadapter->ps_state != PS_STATE_AWAKE || (pmadapter->tx_lock_flag == MTRUE))
                break;
         ...
         * Sleep Request event.
         */
        if ((pmadapter->ps_state == PS_STATE_SLEEP) || (pmadapter->ps_state == PS_STATE_PRE_SLEEP) ||
            (pmadapter->ps_state == PS_STATE_SLEEP_CFM) || (pmadapter->tx_lock_flag == MTRUE))
            continue;
        ...
        if (!pmadapter->scan_processing && !pmadapter->data_sent && !wlan_11h_radar_detected_tx_blocked(pmadapter) &&
        ...
        if (!pmadapter->scan_processing && !pmadapter->data_sent && !wlan_wmm_lists_empty(pmadapter) &&
            !wlan_11h_radar_detected_tx_blocked(pmadapter))
        {
...
#ifdef STA_SUPPORT
        if (pmadapter->delay_null_pkt && !pmadapter->cmd_sent && !pmadapter->curr_cmd &&
            !IS_COMMAND_PENDING(pmadapter) && wlan_bypass_tx_list_empty(pmadapter) && wlan_wmm_lists_empty(pmadapter))
        {
            if (wlan_send_null_packet(wlan_get_priv(pmadapter, MLAN_BSS_ROLE_STA),
                                      MRVDRV_TxPD_POWER_MGMT_NULL_PACKET | MRVDRV_TxPD_POWER_MGMT_LAST_PACKET) ==
                MLAN_STATUS_SUCCESS)
            {
...
mlan_status mlan_send_packet(IN t_void *pmlan_adapter, IN pmlan_buffer pmbuf)
{
    ...
    if (((pmadapter->priv[pmbuf->bss_index]->port_ctrl_mode == MTRUE) &&
         ((mlan_ntohs(*(t_u16 *)&pmbuf->pbuf[pmbuf->data_offset + MLAN_ETHER_PKT_TYPE_OFFSET]) ==
 ...
 *  @return			MLAN_STATUS_SUCCESS
 */
mlan_status mlan_recv_packet_complete(IN t_void *pmlan_adapter, IN pmlan_buffer pmbuf, IN mlan_status status)
{
    mlan_adapter *pmadapter = (mlan_adapter *)pmlan_adapter;
 ...
 */
t_u8 mlan_select_wmm_queue(IN t_void *pmlan_adapter, IN t_u8 bss_num, IN t_u8 tid)
{
    mlan_adapter *pmadapter = (mlan_adapter *)pmlan_adapter;
 ...
 *  @return         N/A
 */
t_void mlan_interrupt(IN t_void *adapter)
{
