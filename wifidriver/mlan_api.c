...
int wifi_send_remain_on_channel_cmd(unsigned int bss_type, wifi_remain_on_channel_t *remain_on_channel)
{
    ...
    mlan_status rv =
        wlan_ops_sta_prepare_cmd(pmpriv, HostCmd_CMD_802_11_REMAIN_ON_CHANNEL,
...
#else
    misc = (mlan_ds_misc_cfg *)OSA_MemoryPoolAllocate(buf_3072_MemoryPool);
#endif
    if (misc == NULL)
    {
        wifi_put_command_lock();
                       ...
                       custom_ie *assocresp_ies_data)
{
    ...
    status = wlan_ops_uap_prepare_cmd((mlan_private *)mlan_adap->priv[bss_type], HOST_CMD_APCMD_SYS_CONFIGURE,
    ...
    const void *cmd_buf, uint32_t cmd_buf_len, void *resp_buf, uint32_t resp_buf_len, uint32_t *reqd_resp_len)
{
    ...
    bss_type = (mlan_bss_type)HostCmd_GET_BSS_TYPE(cmd->seq_num);
                                ...
                                const t_u16 sae_data_len)
{
    ...
    if (pmpriv->auth_flag == 0)
    {
        ...
        if (mlan_adap->remain_on_channel == true)
        {
            ...
            {
                mlan_adap->remain_on_channel = false;
            }
        }
        status = wifi_remain_on_channel(bss_type, true, channel, 6000);
        if (status != WM_SUCCESS)
        {
...
int wifi_set_p2p_mode_config(uint16_t mode_value)
{
    ...
    cmd->seq_num = wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[MLAN_BSS_TYPE_WIFIDIRECT]);
    cmd->result  = 0x0;
...
int wifi_set_ips_config(mlan_bss_type interface, int option)
{
    ...
    cmd->seq_num = wifi_get_cmd_seq_num((mlan_private *)mlan_adap->priv[interface]);
    cmd->size    = cmd_size;
