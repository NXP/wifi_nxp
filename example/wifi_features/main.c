...
int wlan_event_callback(enum wlan_event_reason reason, void *data)
{
    ...
    switch (reason)
    {
        case WLAN_REASON_INITIALIZED:
            ...
            ret = wlan_features_cli_init();

            if (ret != WM_SUCCESS)
            {
                PRINTF("Failed to initialize WLAN Features\r\n");
                return 0;
            }
            PRINTF("WLAN Features are initialized\r\n");
            printSeparator();
