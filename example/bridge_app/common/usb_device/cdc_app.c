...
{
    s_cdcVcom.hwTick++;
    USB_DeviceUpdateHwTick(s_cdcVcom.deviceHandle, s_cdcVcom.hwTick);
}
...
usb_status_t USB_DeviceCdcVcomCallback(class_handle_t handle, uint32_t event, void *param)
{
    ...
    defined(FSL_FEATURE_USB_KHCI_USB_RAM) && (FSL_FEATURE_USB_KHCI_USB_RAM > 0U)
                ...
                {
#ifdef CONFIG_USB_BRIDGE
                    usb_save_recv_data(&s_currRecvBuf[0], s_recvSize);
#endif
                }
    ...
    (defined(USB_DEVICE_CONFIG_LPCIP3511FS) && (USB_DEVICE_CONFIG_LPCIP3511FS > 0U))
        ...
        case kUSB_DeviceEventSuspend:
        {
            ...
            if (s_cdcVcom.attach)
            {
                ...
                s_cdcVcom.suspend   = kStatus_StartSuspend;
                error               = kStatus_USB_Success;
            }
        }
        break;
...
void usb_pm_task(void *handle)
{
    while (1)
    {
        if (s_cdcVcom.isResume)
        {
            s_cdcVcom.isResume = 0U;
            if (kStatus_Idle != s_cdcVcom.suspend)
            {
                s_cdcVcom.suspend = kStatus_Resumed;
            }
        }

        switch (s_cdcVcom.suspend)
        {
            case kStatus_Idle:
                break;
...
TaskHandle_t usb_pm_task_thread;
...
int usb_device_task_init()
{
    ...
    if (xTaskCreate(usb_pm_task, "usb pm task", 2000L / sizeof(portSTACK_TYPE), NULL, OS_PRIO_4, &usb_pm_task_thread) !=
        pdPASS)
    {
        usb_echo("create usb pm task task error\r\n");
...
void usb_device_app_deinit(void)
{
    vTaskSuspendAll();
    os_thread_delete(&s_cdcVcom.deviceTaskHandle);
    os_thread_delete(&usb_pm_task_thread);
    xTaskResumeAll();
...
void usb_device_app_reinit(void)
{
