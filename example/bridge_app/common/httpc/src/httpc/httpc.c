 ...
 * if failed
 */
static inline int httpc_tcp_connect(
    int *sockfd, char *hostname, uint16_t port, int retry_cnt, unsigned int socket_timeout)

{
    struct sockaddr addr;
    int r = tcp_socket(sockfd, &addr, hostname, port, retry_cnt, socket_timeout);
    if (r != WM_SUCCESS)
    {
...
int http_open_session(http_session_t *handle, const char *hostname, const httpc_cfg_t *httpc_cfg)
{
    ...
    r = httpc_tcp_connect(&s->sockfd, (char *)url.hostname, url.portno, s->httpc_cfg.retry_cnt,
