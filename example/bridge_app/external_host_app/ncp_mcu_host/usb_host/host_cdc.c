...
uint8_t usb_host_recv_buff[USB_HOST_RECV_BUFF_LEN];
    ...
    if (cdcInstance->deviceState != cdcInstance->previousState)
    {
        ...
        switch (cdcInstance->deviceState)
        {
            ...
            case kStatus_DEV_Detached:
                ...
                cdcInstance->suspendResumeState  = kStatus_Idle;
                cdcInstance->supportRemoteWakeup = 0U;
                cdcInstance->isSetRemoteWakeup   = 0U;
                cdcInstance->suspendBus          = 0U;
            ...
            for (interface_index = 0; interface_index < configuration->interfaceCount; ++interface_index)
            {
                ...
                else
                {
                    ...
                    if (g_cdc.deviceState == kStatus_DEV_Idle)
                    {
                        ...
                        {
                            g_cdc.supportRemoteWakeup = 1U;
                        }
                        else
                        {
                            g_cdc.supportRemoteWakeup = 0U;
                        }
                        status = kStatus_USB_Success;
                    }
                    else
...
#ifdef CONFIG_USB_BRIDGE
/*Recv usb data*/
void usb_recv_task(void *param)
{
    while (1)
    {
        usb_status_t status                = kStatus_USB_Success;
        cdc_instance_struct_t *cdcInstance = (cdc_instance_struct_t *)&g_cdc;
        if (usb_interface_flag)
        {
            /* run state */
            switch (cdcInstance->runState)
            {
                case kUSB_HostCdcRunIdle:
                    if (g_AttachFlag)
                    {
                        /*Poll recv next packet*/
                        USB_HostCdcDataRecv(g_cdc.classHandle, (uint8_t *)&usb_host_recv_buff[0],
                                            USB_HOST_RECV_BUFF_LEN, USB_HostCdcDataInCb, &g_cdc);
                        get_usb_host_recv_pipe_sem();
                    }
                    break;
                ...
                case kUSB_HostCdcRunGetStateDone:
                    ...
                    cdcInstance->runState     = kUSB_HostCdcRunIdle;
#if USB_HOST_UART_SUPPORT_HW_FLOW
                    USB_HostCdcSetAcmCtrlState(cdcInstance->classHandle, 1, 1, USB_HostCdcControlCallback,
                                               (void *)cdcInstance);
#else
                    cdcInstance->runState = kUSB_HostCdcRunSetCtrlStateDone;
#endif
